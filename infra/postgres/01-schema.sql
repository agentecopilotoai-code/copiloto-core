...
create table app.resources (
  ...
  tenant_id uuid not null references app.tenants(id) on delete cascade,
  vertical_code text not null check (vertical_code in ('field_service','beauty','pet_grooming')),
  resource_type text not null check (resource_type in ('technician','chair','stylist','groomer','room','vehicle')),
  code text not null,
  name text not null,
...
create table app.prompt_templates (
  ...
  vertical_code text not null check (vertical_code in ('field_service','beauty','pet_grooming','common')),
