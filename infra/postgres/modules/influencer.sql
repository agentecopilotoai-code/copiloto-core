-- ============================================================================
-- Módulo Influencer / Ravit Studio — schema completo (idempotente)
-- ============================================================================
-- Se carga via bootstrap.sh --module=influencer DESPUÉS del core.
-- Asume que app.users, app.tenants, app.tenant_modules, app.touch_updated_at
-- y app.current_tenant_id() ya existen (definidos en 10-core.sql).
-- 
-- Crea schema 'influencer.*' aislado del core. RLS por tenant_id en todas
-- las tablas. Activación opt-in via app.tenant_modules.module='influencer'.
-- ============================================================================

-- Grants del schema (extraído de 01-schema.sql legacy)
...
-- UI, conservamos su decisión. En prod, los modules se activan via
-- PATCH /v1/platform/tenant-modules/{tenant}/influencer (con MFA + auditoría).
insert into app.tenant_modules (tenant_id, module, enabled, activated_at)
values ('11111111-1111-1111-1111-111111111111', 'influencer', true, now())
