...
class TestTRDHandlers:
    ...
    def test_listar(self, conn, client):
        ...
        r = client.get('/v1/gd/trd/versiones')
        assert r.status_code == 200

    def test_listar_con_filtros(self, conn, client):
        conn.fetch.return_value = []
        r = client.get('/v1/gd/trd/versiones?estado=vigente&limit=10')
        assert r.status_code == 200

    def test_detalle_ok(self, conn, client):
        conn.fetchrow.return_value = _trd_dict()
        r = client.get(f'/v1/gd/trd/versiones/{uuid4()}')
        assert r.status_code == 200
    ...
    def test_activar_ok(self, conn, client):
        ...
        r = client.post(
            f'/v1/gd/trd/versiones/{uuid4()}/activar', json={},
        )
        assert r.status_code == 200
...
class TestSeriesHandlers:
    ...
    def test_listar(self, conn, client):
        conn.fetch.return_value = []
        r = client.get(f'/v1/gd/trd/versiones/{uuid4()}/series')
        assert r.status_code == 200

    def test_listar_con_estado(self, conn, client):
        conn.fetch.return_value = []
        r = client.get(
            f'/v1/gd/trd/versiones/{uuid4()}/series?estado=activa',
        )
        assert r.status_code == 200
...
class TestSubseriesHandlers:
    ...
    def test_listar(self, conn, client):
        conn.fetch.return_value = []
        r = client.get(f'/v1/gd/trd/series/{uuid4()}/subseries')
        assert r.status_code == 200
...
class TestTiposDocHandlers:
    ...
    def test_listar(self, conn, client):
        conn.fetch.return_value = []
        r = client.get(
            f'/v1/gd/trd/subseries/{uuid4()}/tipos-documentales',
        )
        assert r.status_code == 200
...
class TestTVDHandlers:
    ...
    def test_listar(self, conn, client):
        conn.fetch.return_value = []
        r = client.get('/v1/gd/tvd/versiones')
        assert r.status_code == 200

    def test_listar_con_estado(self, conn, client):
        conn.fetch.return_value = []
        r = client.get('/v1/gd/tvd/versiones?estado=vigente&limit=10')
        assert r.status_code == 200
    ...
    def test_activar_ok(self, conn, client):
        ...
        r = client.post(
            f'/v1/gd/tvd/versiones/{uuid4()}/activar',
        )
        assert r.status_code == 200
...
class TestAsociacionHandlers:
    def test_asociar_ok(self, conn, client):
        ...
        r = client.post(
            f'/v1/gd/dependencias/{dep_id}/codigos-documentales',
    ...
    def test_asociar_mismatch(self, conn, client):
        r = client.post(
            f'/v1/gd/dependencias/{uuid4()}/codigos-documentales',
...
class TestClasificacionHandlers:
    def test_clasificar_nueva(self, conn, client):
        ...
        conn.fetchval.return_value = None
        r = client.post(
            '/v1/gd/clasificacion-documental',
    ...
    def test_consultar_vigente(self, conn, client):
        ...
        r = client.get(
            f'/v1/gd/clasificacion-documental?entidad_tipo=radicado'
            f'&entidad_id={uuid4()}',
        )
    ...
    def test_consultar_sin_clasificacion(self, conn, client):
        ...
        conn.fetch.return_value = []
        r = client.get(
            f'/v1/gd/clasificacion-documental?entidad_tipo=pqrsd'
            f'&entidad_id={uuid4()}',
        )
        assert r.status_code == 200
    ...
    def test_consultar_entidad_invalida(self, conn, client):
        r = client.get(
            f'/v1/gd/clasificacion-documental?entidad_tipo=invalido'
            f'&entidad_id={uuid4()}',
        )
        assert r.status_code == 422
