...
def test_resolve_jwk_unknown_kid_after_refresh_raises():
    ...
    from copiloto_core.core import security  # noqa: PLC0415
...
def test_decode_user_token_uses_local_when_no_domain():
    from copiloto_core.core.security import _decode_user_token
    settings = SimpleNamespace(
...
def test_require_platform_owner_anonymous_401():
    from copiloto_core.core.security import require_platform_owner
...
def test_require_service_rejects_user():
    from copiloto_core.core.security import require_service
...
def test_require_min_role_anonymous_401():
    from copiloto_core.core.security import require_min_role
...
def test_require_mfa_anonymous_401(monkeypatch):
    from copiloto_core.core.security import require_mfa_for_privileged
    req = _request_with_state(actor_type='anonymous')
    with pytest.raises(HTTPException) as exc:
...
def test_clear_jwks_cache_empties_cache():
    from copiloto_core.core.security import clear_jwks_cache, _jwks_cache
...
def test_enforce_session_not_revoked_no_pool(monkeypatch):
    from copiloto_core.core.security import _enforce_session_not_revoked
...
def test_enforce_session_not_revoked_db_error_fail_open(monkeypatch):
    ...
    monkeypatch.setitem(sys.modules, 'copiloto_core.db.pool', SimpleNamespace(db=_MockPool()))
    # Should not raise
    try:
...
def test_decode_local_token_missing_exp_raises():
    ...
    from copiloto_core.core.security import _decode_local_token  # noqa: PLC0415
