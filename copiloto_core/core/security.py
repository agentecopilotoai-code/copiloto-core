...
# desde aquí (`from copiloto_core.core.security import ROLE_LEVELS, PRIVILEGED_ROLES`).
...
async def decode_auth0_id_token(
    ...
    *,
    ...
    Usado por el BFF admin (`copiloto_core.admin.routes`) para extraer claims del
...
async def _enforce_session_not_revoked(session_id: str) -> None:
    ...
    Import lazy de `copiloto_core.db.pool` para evitar ciclo (la pool importa security
...
def has_jwt_role(roles: list[str], minimum_role: str) -> bool:
    ...
    ``ensure_tenant_role`` in ``copiloto_core.api.v1.routes`` uses this to apply the JWT
