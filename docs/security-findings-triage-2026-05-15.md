...
- **VÁLIDO (código vulnerable persiste):** 5 (0 high + 5 low) — actualizado 2026-05-15 tras PR SEC-008 + PR SEC-009 + re-spot-check de `ddce83b1` (cloud LLM DoS, RESOLVED por TASK-0086) + PR de runbook fix (RESOLVED `6317cdc8`).
- **RESOLVED por TASK-0077..0086:** 28 (23 high + 5 low) — incluye `ddce83b1` re-clasificado tras re-spot-check.
...
| ddce83b1 | Blocking cloud LLM classifier enables webhook DoS | **RESOLVED-TASK-0086** | `app/services/intent_classifier.py:161-260` | SEC-012 cerrado retroactivamente | TASK-0086 / BUG09 reemplazó `anthropic.Anthropic` / `openai.OpenAI` síncronos por `AsyncAnthropic` / `AsyncOpenAI` con `timeout=settings.cloud_llm_timeout_seconds` del SDK + `asyncio.wait_for(..., timeout=hard_deadline)` como defensa en profundidad. Tanto `asyncio.TimeoutError` como excepciones del provider degradan a fallback Ollama (también con timeout). Re-spot-checked 2026-05-15: el classifier ya no bloquea el event loop. |
...
| 6317cdc8 | Runbook can leak tenant export to consent complainants | **RESOLVED-SEC-010-RUNBOOK-PR** | `docs/runbooks/consent-violation-claim.md:122-152` | Cerrado vía PR de runbook fix | El runbook ahora prohíbe usar `data-export` para extractos contact-scoped (los query params eran silently ignored y la response es tenant-wide). El operador compone el extracto via SQL ad-hoc hasta que `SEC-010-EXPORT-FU` agregue el endpoint contact-scoped real con audit + signature. |
