...
### TASK-0090 — Auditoría visual UI Influencer vs HTMLs del diseñador (alinear gaps de estilo y estructura)

- **Estado:** PENDING
- **Motivación:** Las tareas UI-INFLU-003..016 se marcaron DONE entregando *componentes funcionales* pero **sin auditoría visual contra los HTMLs originales** del diseñador en `docs/influencer/*.html`. PR #49 (end-to-end wiring) cableó callbacks/rutas/fetches pero **NO tocó el aspecto visual** de los componentes presentacionales existentes. El usuario reportó (sesión 2026-05-21) que comparando los screenshots de diseño contra la app actual hay drift estructural visible:
  1. **Sidebar del shell influencer (`InfluencerShell` / `INFLUENCER_NAV`) no es contextual al personaje activo.** El diseño (HTMLs 02, 04, 05) muestra un sidebar que cuando hay personaje activo expone "PERSONAJE ACTIVO Sofía Vega" arriba + sub-nav **del estudio del personaje** (Estudio / Feed · Posts publicados / Calendario · Programación / Stats · Rendimiento) + sección **GLOBAL** (Casting / Biblioteca / Créditos). El actual es plano y estático: secciones Estudio / Producción / Recursos con 4 items totales (`influencer-casting`, `influencer-calendar`, `influencer-library`, `influencer-credits`).
  2. **Vista "Estudio del personaje" (HTML 02) divergente.** El diseño tiene HERO con foto del personaje a la izquierda (grande, full-bleed con badges sobre la imagen "● ACTIVO · 12 PROGRAMADOS"), meta a la derecha (nombre, handle, descripción, tags de categoría como `Cálida / Cercana / Aspiracional / Resort wear / Joyería / Hospitality`, plataformas conectadas con followers), 4 KpiTiles grandes en cards (Posts publicados, Alcance total, Engagement, Próximo post), tabs (Estudio / Feed·184 / Calendario·12 / Stats), sección "¿Qué quieres crear con Sofía?" con 5 cards de tipo de contenido (Foto / Reel [TOP] / Carrusel / Historia / Anuncio [NEW]) + galería de generaciones recientes. El actual `PersonaStudio.jsx` (186 LOC) tiene KpiTiles + next_post + recent_generations, **falta** hero photo, tabs internas, sección "¿Qué quieres crear?" con cards de kinds.
  3. **CreditsModule.jsx (de PR #49) es página completa cuando el diseño lo trata como widget del sidebar.** El HTML 02 muestra "Créditos · 248" como item del sidebar global con badge numérico, no como vista de página dedicada. La implementación actual (que el agente improvisó en PR #49 sin HTML de referencia) tiene PageHeader + 3 KpiTiles + Card de pricing + DataTable de transacciones + Modal de top-up — es razonable pero NO sigue el patrón visual mostrado.
  4. **Generate (HTML 04) — layout vertical de cards + composer + panel derecho de Ajustes pegado.** El diseño tiene: header con avatar + grupo de personajes, 5 cards horizontales de KINDS con badges (HOT/NEW), composer con textarea + botones [Referencia/Producto/Plantilla], y panel **fijo a la derecha** con Ajustes (Formato 1:1/4:5/9:16/16:9, Cantidad con slider 1-10, Estilo Visual dropdown, Locación upload, Modo seguro toggle), galería de "última generación" en grid. `Generate.jsx` (216 LOC) tiene los KINDS y ajustes funcionalmente pero no se validó el layout pixel-perfect.
  5. **Calendar (HTML 05) — vista semanal con grid de horas (08:00–20:00) en columna izquierda + posts coloreados por persona en su slot + panel lateral derecho con preview del post seleccionado** (foto + caption + plataformas + acciones Aprobar/Editar/Reprogramar) + filtros de personas como chips arriba (con avatares). `Calendar.jsx` (193 LOC) tiene `byDay` + `drawerPost` + filtros — probablemente OK funcionalmente pero NO se validó.
  6. **Wizard Paso 4 (Voz, HTML 03d) y Paso 5 (Plataformas, HTML 03e) son ricos en UI:**
     - Paso 4: Stepper visual de 5 pasos con checks, reproductor de audio sample con waveform, lista de captions generados con esa voz (IG·FOTO, TIKTOK·REEL, IG·STORY con timestamps), panel derecho con Tono base (6 cards: Cálida/Profesional/Juguetona/Aspiracional/Irreverente/Editorial), sliders de Energía/Formalidad/Uso de emojis con marks (Nunca/Algunos/Bastantes/Muchísimos), upload de voz clonada, chips de palabras prohibidas.
     - Paso 5: vista previa de Instagram con grid de fotos (mockup real), cuentas conectadas como cards con toggles (IG·Diario, TikTok·5/sem, etc.), modo de publicación con 4 toggles, cadencia recomendada con cálculo de créditos/semana.
     Los componentes actuales `Step4Voice.jsx` (193 LOC) y `Step5Platforms.jsx` (202 LOC) cubren la funcionalidad pero **NO se validó pixel-perfect contra el HTML**.
- **Decisión de scope (confirmada con el usuario, 2026-05-21):**
  - Esta task es **una auditoría completa screen-por-screen** contra los HTMLs de `docs/influencer/*.html`. NO es un rediseño desde cero — los componentes existentes son la base; identificamos qué falta o qué difiere y lo aplicamos.
  - Cada gap visible (estructura, layout, tipografía, colores, espaciado) se compara con el HTML del diseñador como fuente de verdad. Los HTMLs ya fueron validados por el usuario antes de empezar UI-INFLU.
  - **Out of scope explícitos:**
    - No tocar la API del backend (ya está cableada por PR #49).
    - No cambiar los endpoints ni los shapes de respuesta.
    - No rediseñar el shell del tenant (`TenantShell`) — solo el `InfluencerShell` y vistas internas del módulo Influencer.
- **Alcance por screen (lista priorizada por visibilidad):**
  1. **`InfluencerShell` + `INFLUENCER_NAV` contextual** — agregar selector de persona activa arriba del sidebar; cambiar `INFLUENCER_NAV` a estructura dual ("PERSONAJE ACTIVO" sub-nav + sección GLOBAL); el sub-nav del personaje (Estudio / Feed / Calendario · Programación / Stats) solo aparece cuando hay personaje seleccionado (state del path o context).
  2. **`PersonaStudio.jsx` rediseñado** — agregar hero photo con badges, meta lateral con tags y plataformas conectadas (con followers), 4 KpiTiles grandes en cards, tabs Estudio/Feed/Calendario/Stats, sección "¿Qué quieres crear con [Nombre]?" con cards de kinds que linkean a Generate, galería "Generaciones recientes" con preview de última N.
  3. **`CreditsModule.jsx` rehecho** — adoptar el patrón del diseño (widget compacto en sidebar con badge + vista expandida cuando se clickea). Mover la página de detalle a un drawer/modal o vista más compacta que la actual.
  4. **`Generate.jsx` validación pixel-perfect** — comparar layout con HTML 04, ajustar ancho del panel derecho de Ajustes, asegurar que los 5 KINDS tengan los badges correctos (HOT/NEW/etc.), validar que el composer tenga los botones [Referencia/Producto/Plantilla].
  5. **`Calendar.jsx` validación pixel-perfect** — comparar con HTML 05, asegurar filtros de personas con avatares, panel lateral con preview completo del post.
  6. **`Step4Voice.jsx` y `Step5Platforms.jsx`** — validar contra HTML 03d y 03e, completar elementos faltantes (preview de IG en Step5, waveform en Step4, etc.).
  7. **Pantallas restantes del Wizard (Steps 1-3)** — validar contra HTML 03a, 03b, 03c.
  8. **Toasts del módulo** — validar contra HTML 03 ("Toasts · todos los tipos").
  9. **Empty states** — validar contra HTML 02 ("Empty states · transversal").
  10. **Casting Home con personajes (HTML 01)** — validar `Casting.jsx`.
- **Trabajo estimado:** ~1500–2500 LOC distribuidos en 6–10 commits, dependiendo del nivel de pixel-perfection. Probablemente 2–3 PRs separados (uno por área: Shell+Sidebar, Personaje detalle+Créditos, Wizard+Calendar).
- **Criterios de aceptación (por screen):**
  - Lado a lado, el screenshot del componente actual debe coincidir con el HTML del diseñador en estructura (mismas secciones, mismos KPIs/filtros/cards, misma jerarquía visual).
  - Estilos (colores, tipografía, espaciado) deben matchear los design tokens de UI-INFLU-001.
  - Todos los CTAs del HTML deben existir como botones/links funcionales (aunque algunos puedan ir a placeholders si la sub-feature aún no existe).
  - `npm run build` + `npm test` siguen verdes.
- **Dependencias:** PR #48 mergeado (usePermissions context-only — los componentes lo necesitan). PR #49 mergeado (wiring end-to-end — sin esto no se pueden testear las pantallas con datos reales).
- **Antecedente:** detectado en la sesión del 2026-05-21, después de que el usuario revisara los screenshots de los HTMLs del diseñador (`docs/influencer/02_Estudio_Sofía`, `04_Generar_contenido`, `05_Calendario`, `03d_Voz`, `03e_Plataformas`) y comparara con el panel real, donde Casting muestra empty state correcto pero el resto de pantallas del módulo divergen del diseño. Decisión de scope: cerrar PR #49 con el wiring funcional + diferir la auditoría visual a esta task TASK-0090 para no inflar #49 con cambios de UI que requieren validación lado a lado.
