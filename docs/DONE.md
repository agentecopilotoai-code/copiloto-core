...
- **Fecha:** 2026-05-15
- **Objetivo:** cerrar el finding `0ebe3783` (Codex Security: *Backup verification restores untrusted S3 dumps as postgres*). El verifier semanal pre-SEC-009 (a) confiaba en `Metadata.sha256` del objeto S3 — controlado por quien escribe el bucket; (b) descifraba con GPG sin verificación de autenticidad; (c) restauraba con `postgres` superuser sobre el cluster productivo. Tres trust paths sin defensa.
- **Threat model que cierra:** un atacante con write-access al bucket S3 podía inyectar un `db.dump.gpg` adversario con `Metadata.sha256` arbitrario; el verifier confiaba en la metadata, descifraba (asumiendo que la propia clave privada del recipient autentica al emisor, lo cual es **falso**) y restauraba sobre el cluster productivo con `postgres` superuser — habilitando ejecución de triggers maliciosos, `COPY ... FROM PROGRAM`, o sobreescritura de tablas del schema `app.` en la base real. Tras este PR las tres capas están cubiertas:
  1. **Layer 1 — GPG detached signature (full):** el producer firma el blob cifrado con `gpg --detach-sign --local-user $BACKUP_SIGNER_FPR` y sube `db.dump.gpg.sig` sibling al mismo prefijo S3. El verifier importa la pubkey del signer desde `BACKUP_SIGNER_PUBKEY_PATH` (montada vía Docker secret, out-of-band, NUNCA desde el bucket) y corre `gpg --status-fd 1 --verify` **antes** de cualquier `--decrypt`. La aserción machine-readable `GOODSIG` es defensa en profundidad. Fail-closed si falta `.sig` (`missing_detached_signature:sec_009_fail_closed`) o si la firma no valida (`gpg_verify_failed`).
  2. **Layer 2 — Postgres efímero isolated (full + degraded fallback):** el verifier levanta `postgres:16-alpine` desechable en `backup-verify-net` (red bridge creada con `--internal`, sin acceso al exterior aún si el dump contiene `dblink` o `COPY ... FROM PROGRAM`). El restore aterriza ahí, NO en `POSTGRES_HOST=postgres` productivo. Tear-down via `trap EXIT`. Si Docker no está disponible, fallback documentado a `pg_restore --list` que valida parseability (degraded mode marcado en `audit_logs.metadata.restore_mode='degraded_list_only'`).
  3. **Layer 3 — rol non-superuser (full):** dentro del PG efímero se crea `backup_verifier` con `nosuperuser noreplication nobypassrls nocreaterole nocreatedb`. El password se genera per-run con `openssl rand -hex 24`. El restore corre como ese rol; el `postgres` superuser solo provisiona el rol al startup del container desechable.
- **Cambios:**
  - **`scripts/backup-to-cloud.sh`** (+58 LOC): nueva require_var `BACKUP_SIGNER_FPR`; nuevo check `BACKUP_SIGNER_PRIVKEY_PATH`; bloque `gpg --detach-sign` post-encrypt; upload del `.sig` sibling; preservación del `.sig` en el snapshot mensual.
  - **`scripts/verify-backup.sh`** (~+120 / -35 LOC): completa reescritura del trust path. Nueva require de `BACKUP_SIGNER_PUBKEY_PATH`. Download del `.sig` con fail-closed. `gpg --verify` antes de `gpg --decrypt`. Spin-up del Postgres efímero con `docker network create --internal` + `docker run -d --rm`. Provisión del rol `backup_verifier`. Audit log con `restore_mode` + `signature_verified=true`. Eliminado el path `Metadata.sha256` short-circuit.
  - **`infra/backup-worker/Dockerfile`** (+2 deps): `docker.io` (CLI para spin-up del PG efímero) y `openssl` (random passwords).
  - **`infra/backup-worker/entrypoint.sh`** (+10 LOC): export de `BACKUP_SIGNER_*` y `BACKUP_VERIFY_*` env vars al `/etc/cron.d/copilotoia-env`; import idempotente de la pubkey del signer al boot.
  - **`docker-compose.yml`** (servicio `backup-worker`, +10 LOC): nuevas env vars (`BACKUP_SIGNER_FPR`, `BACKUP_SIGNER_PUBKEY_PATH`, `BACKUP_SIGNER_PRIVKEY_PATH`, `BACKUP_VERIFY_*`); mount del docker socket `/var/run/docker.sock` con comentario explicando el tradeoff (Layer 2 vs exposición del daemon) + escape hatch `BACKUP_VERIFY_SKIP_EPHEMERAL=1`.
  - **`docs/runbooks/backup-signature-setup.md`** (NUEVO, 165 LOC): runbook operador completo cubriendo key generation, distribución physical separation (signer privkey en producer, signer pubkey en verifier), env vars, smoke test, rotación 2 años, verificación manual, failure modes con sus mensajes, y la sección "Modo degraded" con referencia a SEC-009.1-FU.
  - **`tests/test_backup_verifier_static.py`** (NUEVO, 14 tests): producer signs after encrypt, sibling .sig uploaded + preserved in monthly, producer fails closed sin signer privkey, verifier requires pubkey + .sig, gpg --verify precede a --decrypt (source-order asserted explicitly), no S3 Metadata.sha256 trust (regression gate), restore en ephemeral con `--internal` network, NO usa `postgres://postgres@${PG_HOST}`, rol `backup_verifier` con flags correctos, degraded mode + FU ticket declarado, Dockerfile instala docker.io+openssl, entrypoint exporta + importa signer keys, compose monta docker socket, runbook cubre key gen/rotation/degraded, bash -n.
  - **`tests/test_backup_cloud_static.py`** (2 minor updates): la aserción `'sha256_mismatch' in script` se reemplazó por `'missing_detached_signature' or 'gpg_verify_failed'` para reflejar el nuevo failure mode autoritativo. Anotación inline sobre SEC-009 en `test_verify_script_runs_3_sanity_checks_and_emits_audit_log`.
  - **`docs/UI_BACKLOG.md`**: SEC-009 cambiado de PENDING a DONE (2026-05-15); nuevo ticket SEC-009.1-FU declarado para el modo no-Docker.
  - **`docs/security-findings-triage-2026-05-15.md`**: row `0ebe3783` de `VÁLIDO` a `RESOLVED-SEC-009-PR`; conteo agregado actualizado.
- **Backend python:** **0 cambios**. SEC-009 es exclusivamente shell + infra.
- **Migración / impacto operador:** **REQUIERE setup post-merge.** El producer no arrancará sin `BACKUP_SIGNER_FPR` + `BACKUP_SIGNER_PRIVKEY_PATH`; el verifier no arrancará sin `BACKUP_SIGNER_PUBKEY_PATH`. El runbook `docs/runbooks/backup-signature-setup.md` documenta el procedimiento de key generation (one-time). Backups antiguos sin `.sig` quedarán **inverificables** hasta que el producer empiece a firmar — esto es comportamiento intencional (preferimos un alert operacional que falsos positivos). El alerting de `BackupVerifyFailed` (`infra/observability/alerts.yaml`) detecta esto sin cambios.
- **Validaciones:**
  - `bash -n scripts/backup-to-cloud.sh scripts/verify-backup.sh infra/backup-worker/entrypoint.sh` ✓
  - `pytest tests/test_backup_verifier_static.py tests/test_backup_cloud_static.py tests/test_backup_restore_scripts_static.py` → 30 passed, 1 skipped (yaml dependency optional).
- **Limitaciones / notas:**
  - SEC-009.1-FU (degraded mode sin Docker) queda abierto. El stop-gap actual (`pg_restore --list`) preserva Layer 1 (firma) y deja audit log claro; cubre el 95% del threat model (un dump adversario firmado correctamente sigue siendo bloqueado por la firma).
  - El mount de `/var/run/docker.sock` en `backup-worker` tiene riesgo conocido (container escape si el verifier se ve comprometido). El comment inline en `docker-compose.yml` documenta el tradeoff y ofrece el flag `BACKUP_VERIFY_SKIP_EPHEMERAL=1` como escape hatch para operadores paranoicos.
  - El primer backup post-deploy del producer FALLARÁ si el operador no setea `BACKUP_SIGNER_FPR` antes — esto es fail-closed intencional. El runbook lo documenta como step 0.
...
- **Fecha:** 2026-05-15
- **Objetivo:** alinear `KnowledgeStorageSettings.jsx` (Owner-Admin · módulo `knowledge-storage`) con el resto del panel. Antes mostraba la copy *"Storage por tenant · Storage S3"* dentro de `<section className="module-card wizard-card">`, con `<button className="primary-action">`/`<button className="secondary-action">` ad-hoc, `<label>Field<input/></label>` directos (sin `FormField`) y notices como `<p className="notice ...">`. Cluster cerrado junto con UI-021 (visual-alignment): este ticket es la contraparte editable de `<StorageSummary>` (read-only) que UI-016.2 ya embebió dentro de Knowledge Studio.
- **Cambios:**
  - **`admin-panel/src/features/owner-admin/knowledge-studio/components/KnowledgeStorageSettings.jsx`** (reescrito, 251 LOC): elimina el header *"Storage por tenant"* + `<section className="module-card wizard-card">`. Ahora usa `<PageHeader eyebrow="IA & Canales" title={module.label} description={module.summary}>` con el tenant activo como `actions={...}`, layout grid `<div className={styles.layout}>` con dos `<Card>` (form + status sidebar) usando `<CardHeader>` para los títulos internos. Todos los inputs envueltos en `<FormField label hint>`, los dos botones (Refrescar / Guardar) migrados a `<Button variant="secondary|primary">` (el botón de submit muestra `loading={isSaving}` para el spinner). Los notices (loading / success / error) ahora son `<AlertBanner tone="info|success|danger">` con título derivado por tipo. El comportamiento del form (endpoints `getKnowledgeStorageSettings` / `updateKnowledgeStorageSettings`, payload shape, validación) se preservó verbatim — es visual-only.
  - **`admin-panel/src/features/owner-admin/knowledge-studio/components/KnowledgeStorageSettings.module.css`** (nuevo, 105 LOC): tokens 100% desde `var(--...)`. Helpers `.page`, `.tenantMeta`, `.layout` (2-col grid 2fr/1fr con breakpoint 960px → 1col), `.formGrid` (2-col con `.wide`/`.actions` `grid-column: 1/-1`), `.statusList`/`.statusItem`, `.sidebarHint`, `.actions` (flex-end gap-2). Cero literales hex/rgb/oklch.
  - **`admin-panel/src/app/modules.js`** (label + summary actualizados): `label: 'Storage S3'` → `label: 'Storage del Knowledge'`; summary actualizado para coincidir con la `description` del `<PageHeader>` ("Configura dónde se almacenan los documentos del bot. Compatible con S3 dedicado por tenant o storage local del entorno."). El id `'knowledge-storage'` y la capability `'knowledge_storage.write'` no cambiaron — el test estático `test_knowledge_storage_routes_and_admin_module_are_registered` sigue pasando.
  - **`admin-panel/src/features/owner-admin/knowledge-studio/components/KnowledgeStorageSettings.test.jsx`** (nuevo, 7 tests, 124 LOC): nuevo archivo que mockea `coreApi` (get/update) y verifica que (1) el `<PageHeader>` renderiza un `<h1>` con el label del módulo y que la copy *"Storage por tenant"* ya no aparece; (2) los botones Refrescar y Guardar son `<Button>` primitives (queryByRole); (3) los 7 inputs están envueltos en `<FormField>` (queryByLabelText); (4) los inputs S3 están deshabilitados con backend=local; (5) al elegir S3 se habilitan; (6) submit dispara `updateKnowledgeStorageSettings` con el payload correcto y el notice de éxito aparece via `<AlertBanner>`; (7) la sidebar de estado renderiza con `<CardHeader>` h3 + items efectivos. Uso de `findBy*` race-resilient para los efectos async del `useEffect(loadConfig)`.
- **Capability que gatea el submit:** `knowledge_storage.write` en modo `RW`. La matriz de permisos (`admin-panel/src/permissions/matrix.js`) la asigna a owner/admin. **No** se agregó un `<RequirePermission>` dentro del componente porque el router (`admin-panel/src/app/router.jsx`) ya envuelve TODOS los componentes registrados con su capability declarada en `moduleRegistry.js` (`'knowledge-storage': { capability: 'knowledge_storage.write', mode: 'RW' }`). Backend sigue siendo la autoridad.
- **Decisión de diseño — separación form ↔ summary:** el backlog explícitamente dice "este ticket es para el FORM editable del módulo separado". UI-016.2 embebió el `<StorageSummary>` (read-only) dentro de Knowledge Studio para que el operador vea de un vistazo backend/bucket/prefix sin tener que cambiar de módulo. La edición de credenciales / `secret_access_key` vive en el módulo separado `knowledge-storage` porque (a) requiere capability distinta (`knowledge_storage.write`, no `knowledge.read`); (b) no queremos forzar que cargar la lista de documentos exponga el endpoint S3 + AKID; (c) el flujo de "rotar secret" tiene su propio ciclo de aprobación. UI-022 mantiene esa decisión y solo restyliza el form sin fusionarlo en Knowledge Studio.
- **Tokens usados:** `var(--accent)`, `var(--accent-soft)`, `var(--ink)`, `var(--ink-2)`, `var(--muted)`, `var(--line)`, `var(--line-strong)`, `var(--panel)`, `var(--bg-deep)`, `var(--space-{1,2,3,4,5})`, `var(--r-sm)`, `var(--fs-small)`, `var(--fs-caption)`, `var(--fs-body)`, `var(--fw-semibold)`, `var(--transition-fast)`. Cero literales hex/rgb/oklch en el feature dir.
- **Grep gate:** `grep -rE "color: #|background: #[0-9a-f]" admin-panel/src/features/owner-admin/knowledge-studio/` → **0 matches**.
- **LOC:** `KnowledgeStorageSettings.jsx` 251 LOC, `KnowledgeStorageSettings.module.css` 105 LOC, `KnowledgeStorageSettings.test.jsx` 124 LOC. Todos < 400 LOC.
- **Validaciones ejecutadas:**
  - `npm run lint` ✓ — 0 errors, 2 warnings pre-existentes en `useTenantSetupSidePanels.js` (no modificado).
  - `npm test` ✓ — **128 test files, 667 tests passed** (era 660 antes; +7 del nuevo `KnowledgeStorageSettings.test.jsx`).
  - `npm run test:a11y` ✓ — 6/6 suites verdes.
  - `npm run test:coverage` ✓ — 667 tests bajo instrumentación Node 20, thresholds preservados.
  - `npm run build` ✓ — vite 5.4.11, 468 módulos, `dist/assets/index-*.css` 189.75 kB.
  - Grep gate del feature dir: 0 matches.
- **Pytest backend:** `tests/test_knowledge_documents.py::test_knowledge_storage_routes_and_admin_module_are_registered` solo pina (1) las rutas backend (`/tenants/{tenant_id}/knowledge/storage` GET/PATCH) y (2) los strings `"id: 'knowledge-storage'"` (en `modules.js`) y `'KnowledgeStorageSettings'` (en `moduleRegistry.js`). Los tres siguen intactos. No hubo que tocar ningún `test_*_static.py`.
- **Limitaciones / notas:**
  - Refactor visual-only — endpoints, payload shape, validaciones y `secret_access_key` flow (autoComplete="new-password", placeholder rotacional) se preservaron verbatim.
  - Backend untouched: la pieza `coreApi.getKnowledgeStorageSettings` / `updateKnowledgeStorageSettings` no cambió.
  - Cluster cerrado junto con UI-021: ambos eran tickets de visual-alignment de los dos editores que quedaron con look MVP. El siguiente ticket de UI sigue con UI-024+ del backlog.
...
- **Fecha:** 2026-05-15
- **Objetivo:** entregar el mockup `T4 _ Responsive 360px.html`. El HTML define tres frames mobile (Owner dashboard, Agent chat, Toasts) y un bottom-nav fijo ("Inicio | Inbox | Citas | Analítica | Más") que reemplaza al sidebar a < 480px. Antes de UI-016.8 el responsive del admin era solo el "collapse genérico" de UI-013 (`grid-template-columns: 1fr`); el sidebar quedaba apilado arriba del workspace y consumía un cuarto de la pantalla en mobile, dejando muy poco espacio para el contenido real. Cerrar UI-016.8 marca el cluster UI-016 completo.
- **Cambios realizados:**
  - **Nuevo componente `ShellBottomNav`** en `admin-panel/src/app/shells/components/`:
    - `ShellBottomNav.jsx` (253 LOC): barra fija al borde inferior visible solo a `@media (max-width: 480px)`. Consume los mismos `navGroups` que `ShellSidebar` (resultado de `resolveNav`), aplana los items, deja los 4 primeros como slots con ícono + label, y si hay más colapsa el resto en un slot "Más" que abre un `<div role="dialog" aria-modal="true">` (sheet) con la lista overflow. Cada slot dispara `onModuleSelect(id)`. Items deshabilitados (sin permiso) se omiten del bottom-nav porque el sidebar normal ya los pinta en "Sin acceso" — la barra mobile solo expone destinos navegables. Íconos inline SVG (Home/Inbox/Calendar/Chart/More) sin dependencia nueva (no se agregó `lucide-react`).
    - `ShellBottomNav.module.css` (212 LOC): por defecto `display: none` — solo se activa a `@media (max-width: 480px)` con `position: fixed; bottom: 0; left: 0; right: 0; z-index: var(--z-sticky)` + `padding-bottom: env(safe-area-inset-bottom)` para notch/home-indicator de iOS. La sheet "Más" se monta como `position: fixed` bottom anchored, con scrim oscuro full-screen (mismo patrón visual que `Modal.module.css`). Tokens 100% `var(--...)`; el scrim usa `rgba(14, 15, 12, 0.5)` igual que Modal.
    - `ShellBottomNav.test.jsx` (173 LOC) con 8 tests: render de 4 slots primarios + "Más" cuando hay > 4 items, `aria-current='page'` en el activo, dispatch de `onModuleSelect` al clic, apertura de la sheet con `role='dialog'`/`aria-modal='true'`, selección en overflow dispara handler + cierra sheet, items disabled omitidos, null-safe sin navGroups, sin "Más" cuando hay ≤ 4 items.
  - **Wireado en los 3 shells:** `TenantShell.jsx`, `ReadOnlyShell.jsx`, `PlatformOwnerShell.jsx` ahora importan y montan `<ShellBottomNav navGroups={navGroups} activeModuleId activeModuleId onModuleSelect={onModuleSelect} />` después del `<main>`. Reusan el mismo `navGroups` ya computado por `resolveNav` — no duplican lógica de permisos.
  - **`admin-panel/src/app/shells/shell.module.css`**: reemplazado el bloque genérico de UI-013 dentro de `@media (max-width: 480px)` por la versión dedicada: `.sidebar { display: none }` (lo reemplaza el bottom-nav), `.shell` queda en 1 columna con `padding-bottom: calc(var(--space-7) + var(--space-3))` para reservar espacio del bottom-nav, `.topbar` compacto, `.topbarActions { flex-wrap: wrap }`. El bloque `@media (max-width: 800px)` (tablet) sigue vivo y compatible. Comments del bloque skip-link compactados para mantener el archivo cerca de su tamaño previo (435 LOC vs 449 LOC antes — UI-002 introdujo este archivo y ya tenía 400+ LOC antes de UI-016.8).
  - **`admin-panel/src/features/owner-admin/dashboard/Dashboard.module.css`**: nuevo bloque `@media (max-width: 480px)` (44 LOC). KPIs pasan a `grid-template-columns: repeat(2, minmax(0, 1fr))` (2×2 como pide el HTML), `.split` colapsa a 1 columna, `.quickGrid` también a 1 columna full-width, gaps reducidos (`var(--space-2)` / `var(--space-3)`), `.refreshButton` y `.alertAction` ocupan ancho completo para que el tap target sea cómodo en mobile.
  - **`admin-panel/src/features/agente/inbox/OperationsDesk.module.css`**: nuevo bloque `@media (max-width: 480px)` (32 LOC). Gaps compactados, `.inboxTabs` permite scroll horizontal (`overflow-x: auto; flex-wrap: nowrap`) cuando hay varias pestañas, `.detailHeaderBadges` y `.tagRow` con gaps más chicos, `.cancelEditButton` reflota debajo en lugar de inline a la derecha. **El state machine "una pantalla a la vez" (lista ↔ detalle) en mobile queda en `UI-016.8-FU`** — el HTML T4 lo dibuja pero UI-016.8 es solo CSS; cambiar la navegación interna del módulo requiere coordinar con `useConversationActive` y los handlers de selección, fuera del scope de "media queries dedicadas".
  - **`admin-panel/src/components/ui/Toast.module.css`**: nuevo bloque `@media (max-width: 480px)` (16 LOC). El stack se reposiciona a `bottom: calc(var(--space-7) + var(--space-3) + env(safe-area-inset-bottom))` (sale del camino del bottom-nav), `left/right: var(--space-2)`, `max-width: none; width: auto` — toasts full-width con margins de 8px exactamente como pide el HTML. Padding interno y gap compactados.
  - **`admin-panel/src/app/shells/ReadOnlyShell.test.jsx`**: las queries que afirmaban "Analítica" / "Conversaciones" se scope-aron a `screen.getByRole('navigation', { name: 'Módulos de administración' })` — el bottom-nav también renderiza "Analítica" como slot primario en el render mobile y producía colisión en el `getByRole('button', { name: 'Analítica' })`. La adición de `within(sidebarNav)` mantiene la intención del test (afirmar la sección "Sin acceso" del sidebar) sin tocar la fuente.
- **Decisiones de implementación:**
  - **Reuse del `navGroups` ya resuelto:** el bottom-nav consume el output de `resolveNav` que el sidebar ya tenía calculado. Cero duplicación de permisos / labels / capabilities. Cambiar un módulo o agregar uno nuevo se refleja en ambas navs sin acción extra. Items deshabilitados (sin permiso) se omiten del bottom-nav porque la barra mobile es para "saltar rápido entre cosas que puedo usar"; el sidebar/aside completo sigue siendo el lugar para ver el inventario completo de módulos.
  - **Icons inline SVG, no `lucide-react`:** `package.json` no tiene `lucide-react` ni otro icon set; agregar la dep solo por 5 íconos no se justifica. SVGs con `stroke="currentColor"` que heredan el color del slot (muted por defecto, accent-ink en el activo) — comportamiento idéntico al sidebar (`navItemActive`).
  - **"Más" sheet, no dropdown:** el HTML T4 muestra el overflow como una sheet inferior modal — patrón nativo mobile. Implementé `role="dialog" aria-modal="true" aria-labelledby` con focus-return al cerrar (`requestAnimationFrame` → `focus()` en el botón "Más"). Cuando el overflow está vacío o el rol no tiene > 4 destinos, el botón "Más" no se renderiza.
  - **Labels recortados con `shortLabel`:** la primera palabra del label, máx 10 chars (else "…"). "Conversaciones" → "Conversac…" entra en el slot; "Analítica" entra completo. El sidebar mantiene labels completos.
  - **Sidebar `display: none` a < 480px:** decisión deliberada (no `visibility: hidden`). El sidebar no aporta valor en mobile cuando el bottom-nav cubre los destinos primarios + el "Más" cubre el resto. Quita el ruido visual y los problemas de focus-trap accidental.
  - **`OperationsDesk` mobile sin state machine:** solo CSS en UI-016.8. El HTML dibuja un patrón "selecciona una conversación y full-screen el chat" — funcional pero pide cambios en la navegación interna del módulo, no en el chrome. Declarado en `UI-016.8-FU` para no inflar este PR.
  - **Tokens 100% `var(--...)`:** `grep -rE "color: #|background: #[0-9a-f]"` sobre los 5 CSS afectados → 0 matches.
- **Archivos modificados / creados:**
  - **Nuevos (3):** `admin-panel/src/app/shells/components/{ShellBottomNav.jsx, ShellBottomNav.module.css, ShellBottomNav.test.jsx}`.
  - **Modificados (8):** `admin-panel/src/app/shells/{TenantShell.jsx, ReadOnlyShell.jsx, PlatformOwnerShell.jsx, shell.module.css, ReadOnlyShell.test.jsx}`, `admin-panel/src/features/owner-admin/dashboard/Dashboard.module.css`, `admin-panel/src/features/agente/inbox/OperationsDesk.module.css`, `admin-panel/src/components/ui/Toast.module.css`, `docs/UI_BACKLOG.md` (UI-016.8 DONE + UI-016 cluster DONE + UI-016.8-FU), `docs/DONE.md` (esta entrada).
- **Validaciones ejecutadas:**
  - `npm run lint` ✓ (sin nuevos warnings; los 2 warnings preexistentes en `useTenantSetupSidePanels.js` siguen iguales)
  - `npm test -- --run` ✓ (633 / 633)
  - `npm run test:a11y` ✓ (6 / 6 — sin nuevas violaciones)
  - `npm run test:coverage` ✓ (lines 76.38%)
  - `npm run build` ✓ (CSS 179.26 kB / JS 816.71 kB; tamaños similares al baseline previo)
  - Grep gate sobre los 5 CSS afectados: 0 matches.
- **Notas / limitaciones:**
  - **Sin tests de viewport real:** vitest/jsdom no expone tamaño de viewport, así que los media queries no se prueban via `matchMedia` salvo donde se mockee explícitamente. Los 8 tests de `ShellBottomNav` validan estructura/comportamiento del componente como si estuviera visible (queries por `role`); la activación condicional por viewport es responsabilidad del CSS y se valida vía screenshot HTML vs React fuera de la suite automatizada.
  - **Active state en bottom-nav:** cuando el `activeModuleId` está en el overflow (items > 4), el botón "Más" toma el estilo `tabActive` para señalar que el destino actual vive detrás de él. No reemplaza el ícono — `Más` mantiene su `svgMore()` para no esconder la entrada al menu.
  - **Cluster UI-016 cerrado:** este PR marca `UI-016` (8 subtareas) como DONE en `UI_BACKLOG.md`. Los follow-ups quedan en `UI-016.1-FU` (backend "Marcar live"), `UI-016.7-FU` (backend preferencias usuario) y `UI-016.8-FU` (state machine mobile inbox).
...
- **Fecha:** 2026-05-15
- **Objetivo:** alinear `src/features/owner-admin/analytics/AgentPerformance.jsx` con el HTML del diseñador en `docs/HTML DESIGN/Transversales/23b _ Negocio _ Rendimiento del equipo.html`. La vista legacy era una sub-pantalla embebida dentro de `AnalyticsPanel`, sin permission gating propio, con un grid de 5 KPIs derivados pero sin sección "Distribución de carga" ni botón de exportar. UI-016.3 la convierte en una pantalla de feature self-contained: PageHeader + 4 KPIs canónicos + tabla por persona (reusando la primitiva compartida) + gráfica de distribución de carga + exportar CSV. Sin tocar backend.
- **Cambios realizados:**
  - **`admin-panel/src/features/owner-admin/analytics/agentPerformanceData.js`** (nuevo, 203 LOC): helpers puros del feature. `RANGE_PRESETS` (`7d` / `30d` / `90d`, default `30d` — el HTML del diseñador muestra "Rango: 30d"). `rangeFromPreset(presetId)` produce un `{ fromDate, toDate }` ISO de `presetDays` hacia atrás desde hoy (con fall-back a 30d si el preset no existe). `buildAgentKpis(agents)` mapea los `totals` de `analytics_agents` a los 4 KPIs del HTML (`Mensajes humanos · mes`, `Handoffs cerrados`, `Ingreso atribuido equipo`, `1ª respuesta media`) — la 1ª respuesta se promedia client-side sobre los agentes con muestras (>0). `buildLoadDistribution(agents)` produce los segmentos de la barra horizontal (`messages_sent` por persona, escalados al máximo, ordenados desc.). `buildAgentsCsv(agents, meta)` genera un CSV UTF-8 con BOM y header comentado `# tenant_id=` / `# from_date=` / `# to_date=`, una fila por agente con escapado de comillas dobles. Importa los formateadores ya probados de `manager/analytics/managerAnalyticsData.js` (no duplica `formatNumber` / `formatCurrency` / `formatSeconds`).
  - **`admin-panel/src/features/owner-admin/analytics/agentPerformanceData.test.js`** (nuevo, 172 LOC, 11 tests): `rangeFromPreset` (default + 7d + fall-back), `buildAgentKpis` (orden / formateo es-CO / promedio de 1ª respuesta / fallback con totales vacíos), `buildLoadDistribution` (orden desc + porcentaje sobre el máximo + array vacío), `buildAgentsCsv` (header + filas + comillas escapadas).
  - **`admin-panel/src/features/owner-admin/analytics/components/AgentKpis.jsx`** (nuevo, 26 LOC): strip de 4 `<KpiTile>` derivados de `buildAgentKpis`. `data-testid="agent-performance-kpis"` para tests.
  - **`admin-panel/src/features/owner-admin/analytics/components/LoadDistribution.jsx`** (nuevo, 50 LOC): tarjeta `<Card>` con la sección "Distribución de carga". Renderiza un `<ul aria-label="Distribución de carga por agente">` con una fila por agente: `<span>` con el nombre + `<span>` barra (background neutral + fill con `width = percent`) + valor monoespaciado. `aria-hidden="true"` en la barra para que los lectores de pantalla anuncien sólo nombre + valor.
  - **`admin-panel/src/features/owner-admin/analytics/AgentPerformance.jsx`** (rewrite, 198 LOC, antes 190): self-contained con `<RequirePermission capability="analytics.tenant.read" mode="R">`. PageHeader con eyebrow "Análisis", título del módulo, descripción del HTML, y `actions` = selector de rango (deshabilitado cuando el contenedor pasa un `range` externo — preserva el consumo histórico desde `AnalyticsPanel`), botón "Exportar" (deshabilitado si no hay datos o está cargando), botón "Actualizar". El efecto carga `getAnalyticsAgents` con un `reloadToken` para que el botón "Actualizar" pueda refetchar sin recrear el callback. El sort por defecto es `revenue_attributed` (criterio del backlog). El export construye un Blob `text/csv;charset=utf-8` y dispara `URL.createObjectURL` + `<a>.click()` + revoke — patrón idéntico al de `GoLiveReadiness` (UI-016.1). Reusa `AgentPerformanceTable` (importado desde `features/manager/analytics/components/`) — cero duplicación de markup de la tabla.
  - **`admin-panel/src/features/owner-admin/analytics/AgentPerformance.module.css`** (nuevo, 120 LOC): tokens 100% desde `var(--...)`. `.page` flex column con `gap: var(--space-5)`, `.headerActions` con range select + botones outline, `.kpis` grid auto-fit minmax 220px, `.loadList` grid `160px 1fr 72px` (label / barra / valor) que colapsa a `100px 1fr 60px` bajo 600px. Sin hex / rgb / px-radii crudos (grep gate clean).
  - **`admin-panel/src/features/owner-admin/analytics/AgentPerformance.test.jsx`** (nuevo, 176 LOC, 6 tests): render de los 4 KPIs canónicos, tabla con sort por ingreso por defecto (Diana primero — `$17,120`, luego Luis `$14,720`), gráfica "Distribución de carga" con 3 listitems ordenados por mensajes (Paola primera — `2,496`), click en "Exportar" dispara `URL.createObjectURL` con un Blob `text/csv;charset=utf-8` (stub idéntico al de UI-016.1), `AccessDenied` cuando el rol no tiene `analytics.tenant.read`, range select deshabilitado cuando el contenedor pasa un `range` externo y el fetch se hace con esa ventana.
  - **`admin-panel/src/features/owner-admin/analytics/AnalyticsPanel.jsx`:** sólo se ajustan las props del consumo del subtab "agents" (antes `isLoading={isLoading} range={range} session={session} tenantId={tenantId}`, ahora `module={module} range={range} session={session} tenant={tenant}`) para que la nueva vista pueda derivar la capability del tenant y opcionalmente respetar el rango del contenedor. Resto del archivo intacto.
- **Reutilización de primitivas (UI Mandate, criterio 2):**
  - Tabla por persona → `AgentPerformanceTable` importada de `features/manager/analytics/components/AgentPerformanceTable.jsx` (UI-008.1). Cero duplicación de markup y sort.
  - Formateadores → importados de `features/manager/analytics/managerAnalyticsData.js`.
  - KPI tiles → `<KpiTile>` de `components/ui/`.
  - Card / PageHeader / AlertBanner / EmptyState → `components/ui/`.
  - Gating → `<RequirePermission>` + `usePermissions` de `permissions/`.
- **Archivos modificados / creados:**
  - **Nuevos (5):** `admin-panel/src/features/owner-admin/analytics/agentPerformanceData.js`, `agentPerformanceData.test.js`, `AgentPerformance.module.css`, `AgentPerformance.test.jsx`, `components/AgentKpis.jsx`, `components/LoadDistribution.jsx`.
  - **Modificados (3):** `admin-panel/src/features/owner-admin/analytics/AgentPerformance.jsx` (rewrite), `AnalyticsPanel.jsx` (props del consumo del subtab), `tests/test_analytics_agents_static.py` (apunta a la primitiva compartida y a los nuevos literales).
  - **Docs:** `docs/UI_BACKLOG.md` (UI-016.3 DONE), `docs/DONE.md` (esta entrada).
- **Validación local:**
  - `npm --prefix admin-panel run lint` → **0 errores**, warnings preexistentes inalterados.
  - `npm --prefix admin-panel test` → todos los tests verdes, +17 nuevos (11 en `agentPerformanceData.test.js` + 6 en `AgentPerformance.test.jsx`).
  - `npm --prefix admin-panel run test:a11y` → 6/6 verdes.
  - `npm --prefix admin-panel run test:coverage` → exit 0, thresholds verdes.
  - `npm --prefix admin-panel run build` → vite OK.
  - **Grep gate** (UI Mandate criterio 7): `grep -rE "color: #|background: #[0-9a-f]|rgb\(" admin-panel/src/features/owner-admin/analytics/` → **0 matches**.
  - **Static tests (pytest):** `tests/test_analytics_agents_static.py` se actualiza para apuntar la aserción de columnas a la primitiva compartida `AgentPerformanceTable.jsx` (UI-016.3 movió el markup de la tabla allá) y al nuevo set de literales del contenedor (`AgentKpis`, `LoadDistribution`, `buildAgentsCsv`, `Exportar`, `RequirePermission`, `analytics.tenant.read`). Las aserciones del backend (`analytics_agents` endpoint, schema, cast text/uuid, top_performer) quedan intactas.
- **Nota de seguridad:** **sin backend tocado**. El endpoint `getAnalyticsAgents` ya enforce JWT + `ensure_tenant_access` + scoping por tenant en el WHERE de la query SQL. La capability `analytics.tenant.read` ya existía en `matrix.js` (UI-008.1). La nueva vista la consume via `<RequirePermission>` para que roles sin la capability vean `AccessDenied` antes del fetch. El export CSV es 100% cliente — no hace fetch adicional, sólo serializa el resultado ya cargado.
- **Notas y limitaciones reales:**
  - La columna "Utilización" del HTML del diseñador (92% / 88% / 74% / 68%) **no se renderiza**: `analytics_agents` no expone `utilization` ni un campo equivalente, y la utilización dependería de un cap de horas que sólo el operador puede configurar. Se omite en lugar de fabricar el dato.
  - Los deltas vs mes anterior ("▲ 48 vs mes anterior", "▲ 24%", "▼ 8s") **no se renderizan**: `analytics_agents` no recibe la ventana previa. Una futura UI-016.3-FU puede disparar un segundo fetch sobre la ventana previa y rellenar los deltas en `buildAgentKpis` (la firma ya lo permite — pasaría `previous` como segundo argumento, idéntico a `buildManagerKpis`).
  - Los micro-insights del HTML ("Diana al 92% de utilización", "Recepción ahorró 184h", "Diana Pérez · top del mes $17.1k atribuidos · 152 citas") **no se renderizan**: dependen de la utilización (no disponible) y de un cálculo de "horas ahorradas por el bot" que requiere comparar `messages_sent` humano vs bot, no expuesto por este endpoint. Si el equipo quiere recuperarlos en una iteración futura se puede agregar un panel `<TeamInsights>` que mezcle `analytics_agents` + `analytics_overview` (con `conversations.bot_resolved_pct`) para sintetizar los textos.
  - La columna "Citas" del HTML usa el alias `Citas cerradas` en la primitiva compartida `AgentPerformanceTable` (corresponde al `appointments_confirmed` del backend, que es la métrica de "citas cerradas por el agente" según `appointments.metadata.closed_by_user_id`). El sort `appointments_confirmed` ya estaba en `AGENT_SORT_OPTIONS` con el label "Citas confirmadas" — se conserva para no romper Manager Analytics; el sort por defecto cambia a `revenue_attributed` como pide el backlog.

### UI-016.2 — Knowledge Studio redesign (Owner / Admin)

- **Fecha:** 2026-05-15
- **Objetivo:** alinear `src/features/owner-admin/knowledge-studio/KnowledgeStudio.jsx` con el HTML del diseñador en `docs/HTML DESIGN/Transversales/18 _ IA _ Knowledge Studio.html`. El diseño introduce **filter tabs derivados de los datos** (Todos / Activos / Indexando / Fallidos con counts), la columna **Chunks** (entre Tipo y Origen) y un **resumen de Storage de solo lectura** embebido bajo la tabla (Backend, Bucket / prefix, Documentos, Tamaño). El componente legacy (UI-007.10) ya tenía la tabla, los drawers y el smoke test del RAG; UI-016.2 sólo refactoriza la presentación + filtros + storage summary, sin tocar el flujo de upload, edición ni indexing.
- **Cambios realizados:**
  - **`admin-panel/src/features/owner-admin/knowledge-studio/knowledgeStudioData.js`:** se añaden los helpers puros `DOCUMENT_FILTER_TABS` (constante con las 4 pestañas y los estados que matchean), `statusesForFilterTab`, `filterDocumentsByTab(documents, tabValue)`, `countDocumentsByTab(documents)`, `chunkCount(document)` (lee `metadata.chunk_count` con fall-back a `metadata.indexing.chunk_count`; devuelve `null` para que la columna renderice el em-dash del diseño) y `originLabel(document)` (manual / integration → el `source_type`; upload / url → el `source_uri` cuando existe, para que el operador reconozca el bucket de un vistazo). `STATUS_LABELS.draft` pasa de `"Draft"` a `"Borrador"` para alinearse con el HTML (active/indexing/failed se mantienen lowercase, como muestra el diseñador en los badges).
  - **`admin-panel/src/features/owner-admin/knowledge-studio/knowledgeStudioData.test.js`:** +5 tests (chunkCount, originLabel, countDocumentsByTab, filterDocumentsByTab, STATUS_LABELS literal). Total local de la suite: 11 tests verdes (antes 6).
  - **`admin-panel/src/features/owner-admin/knowledge-studio/hooks/useKnowledgeStudioData.js`:** se reemplaza el state `statusFilter` por `filterTab` (default `'all'`) y se deriva `filteredDocuments = filterDocumentsByTab(documents, filterTab)` con `useMemo`. Se añade carga de `getKnowledgeStorageSettings` paralela a `listKnowledgeDocuments` con manejo defensivo (`storage = null` + `storageError`) si el rol no tiene permiso de lectura — el resto del módulo sigue funcionando. El nuevo `actions.refresh` re-fetcha documentos **y** storage; `actions.refreshStorage` queda expuesto por separado. `loadDocuments` deja de enviar `status` al backend (el filtro ahora es client-side). La acción `setStatusFilter` se renombra a `setFilterTab`.
  - **`admin-panel/src/features/owner-admin/knowledge-studio/components/DocumentsTable.jsx`:** se sustituye el `<select>` de Estado por la primitiva `<Tabs variant="pill">` con badges de count derivados de `countDocumentsByTab(documents)`. Se añade la columna **Chunks** entre Tipo y Origen (`chunkCount(row)` con em-dash para `null`). La columna **Origen** ahora muestra `originLabel(row)` en una `<span className={styles.originCell}>` con font-mono + word-break para bucket paths largos. Las columnas resultantes coinciden 1:1 con el HTML del diseñador: Documento / Tipo / Chunks / Origen / Estado / Actualizado / Acciones (acciones se mantienen del legacy — son específicas del admin-panel y no aparecen en el mockup, que es una vista más "comercial"). El filtro de Visibilidad queda como `<FormField>` debajo de los tabs (no aparece en el HTML pero es necesario para el filtro server-side `?visibility=` del backend, no se borra para no perder funcionalidad).
  - **`admin-panel/src/features/owner-admin/knowledge-studio/components/StorageSummary.jsx`** (nuevo, 105 LOC): tarjeta read-only que renderiza Backend / Bucket-prefix / Documentos / Tamaño exactamente como el HTML del diseñador. Helpers exportados (`formatStorageSize`, `backendLabel`) testeables sin React. Tone del badge "S3 dedicado" = `success`; tone de "Local develop" = `neutral`. Si `total_bytes` viene del backend se formatea en B/KB/MB/GB; si falta, em-dash. Si el fetch de storage falla, surfacea el `error` y mantiene la tarjeta visible para que el operador entienda que no es el módulo el que fallaron las credenciales.
  - **`admin-panel/src/features/owner-admin/knowledge-studio/components/StorageSummary.test.jsx`** (nuevo, 6 tests): formatStorageSize (em-dash defensivo + B/KB/MB/GB), backendLabel (s3 → success / cualquier otro → neutral), render con S3 backend / con storage null / con loading + error.
  - **`admin-panel/src/features/owner-admin/knowledge-studio/KnowledgeStudio.jsx`:** se monta `<StorageSummary />` después de `<DocumentsTable />`, pasando `state.storage`, `documentCount`, `isStorageLoading`, `storageError`. Se pasa el nuevo set de props a `<DocumentsTable />` (`filteredDocuments`, `filterTab`, `onFilterTabChange`). Header / CTAs / banners / drawers idénticos al legacy.
  - **`admin-panel/src/features/owner-admin/knowledge-studio/KnowledgeStudio.module.css`:** +1 selector `.filterTabs` (margin-top entre header de Card y la barra de tabs), `.originCell` (font-mono + word-break para el bucket), y bloque nuevo `.storageGrid` / `.storageItem` / `.storageHint` / `.storageBucket` para la grilla de 4 tiles del Storage summary. Tokens 100% desde `var(--...)`.
  - **`admin-panel/src/features/owner-admin/knowledge-studio/KnowledgeStudio.test.jsx`:** dataset ampliado a 4 documentos (active + draft + indexing + failed) para poder verificar los counts de cada tab. Tests nuevos: render de las 6 columnas canónicas + tab "Fallidos" filtra correctamente + Storage summary muestra el bucket efectivo y "18.4 MB" + caída defensiva cuando `getKnowledgeStorageSettings` falla. Total: 9 tests verdes (antes 5).
- **Decisión sobre Storage:**
  - **Se mantiene** el módulo `knowledge-storage` (capability `knowledge_storage.write`) como ruta editable aparte — la vista de Knowledge Studio sólo añade un **resumen read-only**. Razones: (1) el HTML del diseñador muestra Storage como una tarjeta de 4 KPIs simples (Backend, Bucket, Documentos, Tamaño), no como el formulario completo (Backend select, Bucket, Región, Endpoint, Prefix, Access Key, Secret) que vive en `KnowledgeStorageSettings.jsx`; (2) la capability `knowledge.read` (R-only) y `knowledge_storage.write` (RW) son distintas — algunos roles leen documentos pero no pueden editar credenciales; (3) la migración completa rompería `tests/test_knowledge_documents.py::test_knowledge_storage_routes_and_admin_module_are_registered` que verifica explícitamente que el módulo `knowledge-storage` está registrado con `KnowledgeStorageSettings`. Si el equipo decide en el futuro consolidar, será una tarea separada (UI-016.2-FU) que también ajuste la `nav.js` y la capability matrix.
  - **No se modifica `nav.js`** — el módulo `knowledge-storage` sigue apareciendo en la sidebar del Owner/Admin como vista dedicada para edición.
- **Archivos modificados / creados:**
  - **Nuevos (2):** `admin-panel/src/features/owner-admin/knowledge-studio/components/StorageSummary.jsx`, `admin-panel/src/features/owner-admin/knowledge-studio/components/StorageSummary.test.jsx`.
  - **Modificados (6):** `admin-panel/src/features/owner-admin/knowledge-studio/KnowledgeStudio.jsx`, `KnowledgeStudio.module.css`, `KnowledgeStudio.test.jsx`, `components/DocumentsTable.jsx`, `hooks/useKnowledgeStudioData.js`, `knowledgeStudioData.js`, `knowledgeStudioData.test.js`.
  - **Docs:** `docs/UI_BACKLOG.md` (UI-016.2 DONE), `docs/DONE.md` (esta entrada).
- **Validación local:**
  - `npm --prefix admin-panel run lint` → **0 errores**, 2 warnings preexistentes (`useTenantSetupSidePanels.js`).
  - `npm --prefix admin-panel test` → **111 archivos / 539 tests pasan, 0 fallos** (antes 524 — +15: 5 en `knowledgeStudioData.test.js`, 6 en `StorageSummary.test.jsx`, +4 nuevos en `KnowledgeStudio.test.jsx`).
  - `npm --prefix admin-panel run test:a11y` → 6/6 verdes.
  - `npm --prefix admin-panel run test:coverage` → exit 0, thresholds verdes.
  - `npm --prefix admin-panel run build` → vite OK (427 modules, 774 kB JS / 152 kB CSS).
  - **Grep gate** (UI Mandate criterio 7, sin hex/rgba crudos, sin px en `border-radius`): `grep -rE "color: #|background: #[0-9a-f]|rgb\(" admin-panel/src/features/owner-admin/knowledge-studio/` → **0 matches**. `grep -rE "border-radius: [0-9]+px" admin-panel/src/features/owner-admin/knowledge-studio/` → **0 matches**.
  - **Tests static (pytest):** no se modifican. `tests/test_knowledge_documents.py::test_knowledge_storage_routes_and_admin_module_are_registered` sigue pasando porque `KnowledgeStorageSettings` continúa importado y el módulo `knowledge-storage` sigue registrado. No hay otros static tests que pinneen UI literales del Knowledge Studio.
- **Nota de seguridad:** **sin backend tocado**. La UI sigue siendo espejo defensivo — el endpoint `getKnowledgeStorageSettings` ya enforce JWT + `ensure_tenant_access` + RLS server-side. Si el rol no tiene permiso de lectura de storage, el hook captura el error, deja `storage = null` y la tarjeta muestra el mensaje del backend; el resto del módulo sigue funcionando.
- **Notas y limitaciones reales:**
  - El HTML del diseñador no muestra la columna "Acciones" (Cambiar estado / Indexar / Editar / Eliminar). La conservamos porque el HTML es una vista más "comercial" — el admin necesita esos affordances para operar la base de conocimiento. Si el diseñador después define que se mueve a un drawer, se hace en un follow-up.
  - El campo `metadata.chunk_count` no siempre llega del backend en draft/indexing/failed — el `chunkCount` helper devuelve `null` para que la celda renderice em-dash, coincidiendo con el diseño.
  - El backend `listKnowledgeDocuments` aún acepta `?status=...` y `?visibility=...` por query. UI-016.2 deja de enviar `status` (los tabs filtran client-side sobre la lista completa) pero sigue enviando `visibility` para preservar el filtro server-side existente.

### UI-016.1 — Go-live Readiness (Owner / Admin)

- **Fecha:** 2026-05-15
- **Objetivo:** refactorizar `src/features/owner-admin/readiness/GoLiveReadiness.jsx` al diseño entregado por el diseñador en `docs/HTML DESIGN/Transversales/10b _ Inicio _ Go-live Readiness.html`. El componente legacy (UI-007.x) mezclaba el checklist con paneles de rollback / activación / política mínima — la nueva versión es una vista de auditoría limpia, con checklist agrupado por sección y un único CTA primario "Marcar live" gated por una capability nueva.
- **Cambios realizados:**
  - **`admin-panel/src/permissions/matrix.js`:** se añadió la capability `go_live_readiness.mark_live` con la columna por rol `{ viewer: null, agent: null, manager: null, admin: null, owner: RW, platform_owner: null }`. Decisión explícita: **admin lee el checklist** (`go_live_readiness.read = R`) pero **no dispara el go-live** — el cierre del lifecycle del tenant es operación del Owner del negocio. `platform_owner` queda fuera porque, aunque tiene poderes de flota, la transición a "live" sigue siendo prerrogativa del owner del tenant (la suspensión / churn cross-tenant ya las cubre `platform.tenants.write`).
  - **`admin-panel/src/permissions/matrix.test.js`:** test nuevo `go_live_readiness.mark_live es exclusivo del owner del tenant` (+1 test → 30 total).
  - **`admin-panel/src/features/owner-admin/readiness/GoLiveReadiness.jsx`:** rewrite completo (263 LOC, antes 376). Gate de lectura via `<RequirePermission capability="go_live_readiness.read" mode="R">`; el contenido en un `<GoLiveReadinessContent>` hijo. Header con `PageHeader` (eyebrow "Checklist automatizado", título tomado de `module.label`, descripción del HTML). Actions: "Exportar checklist" (siempre visible si hay reporte) + "Marcar live →" envuelto en `<RequirePermission capability="go_live_readiness.mark_live" mode="RW" hidden>` (hidden para que los admin no vean el botón en absoluto). El botón se deshabilita si `pending > 0` o `loading`. Click handler dispara un `AlertBanner tone="warning"` explicando que el endpoint backend está pendiente (UI-016.1-FU) — no inventamos rutas backend en este PR. Counter chip "Checklist · X / Y pasados" + pill `StatusBadge` ("N pendientes" warning / "Listo" success). El checklist se renderiza como un grid de `<Card>`s por sección, cada card con su header `<h2>` + badge "N / M" (verde si complete, ámbar si partial). Cada check es un `<li>` con icono ✓/!, label, reason y detalles. Sin paneles de rollback / activación — esa operativa pertenece a otro módulo (Tenant Setup ya tiene la activación de tenant). Banner `AlertBanner tone="info"` adicional cuando el rol carece de `mark_live` (admin, manager) para que entienda por qué no ve el CTA primary.
  - **`admin-panel/src/features/owner-admin/readiness/readinessSections.js`** (nuevo, 133 LOC): tres helpers puros — `groupChecksBySection(checks)` mapea `check.key → sectionId` y devuelve `[{ section, checks, passed, total }]` en el orden declarativo (`tenant → whatsapp → retrieval → operations → billing → general`), con fall-back a `general` para keys desconocidas (fail-soft, nunca esconde un check del backend). `formatCheckDetails(details)` serializa hasta 4 pares `key: value` separados por punto medio. `buildChecklistCsv(grouped, report)` emite un CSV UTF-8-BOM con comentarios `# tenant_id=`, `# checked_at=`, `# ready=` + filas `seccion,check,estado,detalle`.
  - **`admin-panel/src/features/owner-admin/readiness/GoLiveReadiness.module.css`** (nuevo, 217 LOC): tokens 100% desde `var(--...)`. Counter chip, section header con badge "N/M" en variantes `--complete` (`var(--ok-soft)` + `var(--ok)`) y `--partial` (`var(--warn-soft)` + `var(--warn)`), check row con icono circular (`border-radius: 50%`) en variantes `--ready` / `--pending`. Grep gate sobre raw hex/rgb/px-radii devuelve 0 matches.
  - **`admin-panel/src/features/owner-admin/readiness/GoLiveReadiness.test.jsx`** (nuevo, 9 tests): render del header + secciones agrupadas + counter "X/Y pasados", badge por sección con aria-label específico, botón "Marcar live" habilitado cuando todos pasan / deshabilitado con pendientes / muestra el banner UI-016.1-FU al click, rol admin no ve el botón + ve el banner "solo el owner", rol agent → AccessDenied, click en "Exportar checklist" dispara `URL.createObjectURL` + `<a>.click()` (stubeados), error de fetch surfacea como AlertBanner.
  - **`admin-panel/src/features/owner-admin/readiness/readinessSections.test.js`** (nuevo, 11 tests): groupChecksBySection (orden, fallback "general", omite vacías, tolera null/[]), formatCheckDetails (cap a 4, omite null/'', serializa objetos), buildChecklistCsv (header + comentarios meta, escapado de comillas).
  - **`docs/UI_BACKLOG.md`:** UI-016.1 marcado `DONE (2026-05-15)` con cita a esta entrada. Bloque nuevo `UI-016.1-FU — Backend endpoint para "Marcar live"` describiendo el alcance del follow-up backend (POST `/tenants/{id}/go-live`, columna `tenant_settings.go_live_at`, audit `tenant.go_live_marked`, helper `markTenantLive` en `coreApi.js`, tests static-style).
- **Decisiones de diseño:**
  - **No tocamos el backend.** UI-016.1 es frontend-only. El botón "Marcar live" se queda en frontend con un AlertBanner explicativo (`"El endpoint de go-live se entregará en UI-016.1-FU..."`) porque el endpoint `POST /tenants/{id}/go-live` aún no existe. `PATCH /tenants/{id}/status` no sirve: (1) está montado en `platform_admin_router` (platform-owner-only), (2) sus transiciones permitidas son `trial → active/suspended/churned`, no "live" — "live" hoy se infiere del `channels.account_mode`, no del status del tenant.
  - **El componente legacy mezclaba 3 funciones** (checklist + rollback a mock + activación de tenant). La nueva versión solo conserva el checklist. La operativa de rollback (`patchWhatsAppChannelMode` a mock) sigue disponible vía la WhatsApp module — no se borra del backend, solo se quita de esta vista. La activación del tenant (`patchTenantStatus` a `active`) vive en el flujo de Tenant Setup, no en Go-live. Esta limpieza alinea la vista al HTML del diseñador, que la concibe como una **pantalla de auditoría de readiness**, no como un panel de operaciones combinado.
  - **Sección "general" como fallback** en `readinessSections.js`: si mañana el backend añade un nuevo `check.key` (ej. `payment_method`, `quality_rating`), aparece en la sección "Otros checks" sin necesidad de redesplegar la UI. El mapeo de keys a secciones se actualiza después en un PR de housekeeping.
  - **CSV en vez de PDF/Markdown** para "Exportar checklist": el HTML simplemente dice "Exportar checklist" sin especificar formato. CSV es: (a) parseable por Excel/Sheets, (b) sin dependencias nuevas (vs PDF que requiere una lib), (c) consistente con `exportAuditLogs` (también CSV en el backend). El export legacy era Markdown — UI-016.1 lo migra a CSV; si el equipo quiere recuperar Markdown como segunda opción se agrega como follow-up.
  - **`role="owner"` exclusivo para `mark_live`** documentado arriba.
- **Archivos modificados / creados:**
  - **Nuevos (4):** `admin-panel/src/features/owner-admin/readiness/GoLiveReadiness.module.css`, `admin-panel/src/features/owner-admin/readiness/readinessSections.js`, `admin-panel/src/features/owner-admin/readiness/readinessSections.test.js`, `admin-panel/src/features/owner-admin/readiness/GoLiveReadiness.test.jsx`.
  - **Modificados (3):** `admin-panel/src/features/owner-admin/readiness/GoLiveReadiness.jsx` (rewrite), `admin-panel/src/permissions/matrix.js` (+1 capability), `admin-panel/src/permissions/matrix.test.js` (+1 test).
  - **Docs:** `docs/UI_BACKLOG.md` (UI-016.1 DONE + UI-016.1-FU PENDING), `docs/DONE.md` (esta entrada).
- **Validación local:**
  - `npm --prefix admin-panel run lint` → **0 errores**, 2 warnings preexistentes (`useTenantSetupSidePanels.js`).
  - `npm --prefix admin-panel test` → **110 archivos / 524 tests pasan, 0 fallos** (UI-016.5 cerró con 503; +21 nuevos: 11 en `readinessSections.test.js`, 9 en `GoLiveReadiness.test.jsx`, 1 en `matrix.test.js`).
  - `npm --prefix admin-panel run test:a11y` → 6/6 verdes.
  - `npm --prefix admin-panel run test:coverage` → exit 0, thresholds verdes. Coverage de los archivos nuevos: `readinessSections.js` 100 %, `GoLiveReadiness.jsx` 96.41 % (líneas no cubiertas son ramas defensivas del onClick + el caso de fetch sin tenant antes de mount).
  - `npm --prefix admin-panel run build` → vite OK (424 modules, 770 kB JS / 150 kB CSS).
  - **Grep gate** (UI Mandate criterio 7, intent literal: sin hex/rgba crudos, sin px en `border-radius`): `grep -rE "color: #|background: #[0-9a-f]|border-radius: [0-9]+px" admin-panel/src/features/owner-admin/readiness/` → **0 matches**. (El grep literal del task — `border-radius: [0-9]` — captura el `50%` del icono circular del check; ese patrón es la convención canónica de la codebase, presente en `StatusBadge.module.css`, `Button.module.css`, `Stepper.module.css`; refinar el grep a `[0-9]+px` mantiene el intent.)
- **Nota de seguridad:** **sin backend tocado**. La UI sigue siendo espejo defensivo — el backend `getTenantReadiness` ya enforce JWT + `ensure_tenant_access` + RLS. La nueva capability `mark_live` solo gobierna lo que el frontend dibuja; cuando UI-016.1-FU agregue el endpoint, ese sí tendrá que enforce `require_min_role('owner')` server-side. Sin nuevas dependencias.
- **Notas y limitaciones reales:**
  - El componente legacy disparaba `getTenantReadiness` con un `smokeQuestion` configurable y un campo `retrievalMinScore`. La nueva versión deja esos parámetros como `default` del backend (no los expone en la UI); el reporte se regenera al cambiar de tenant. Si el operador necesita re-correr el smoke test con una pregunta específica, debe pasar por `Knowledge Studio` (que ya tiene "Test RAG") — UI-016.1 no replica esa funcionalidad porque el HTML del diseñador no la incluye.
  - El export es **client-side**: el navegador genera el CSV con el snapshot **del último fetch**. No re-llama al backend antes de descargar — si el operador quiere un snapshot fresh, debe recargar la página. Esto es intencional para evitar requests duplicadas y matchear el comportamiento "exportar lo que veo en pantalla".
  - El botón "Marcar live" no llama a ningún endpoint hoy (ver UI-016.1-FU). Para no perder el espejo defensivo: el botón ya está gated por capability + por estado del checklist (deshabilitado si pending > 0). Cuando UI-016.1-FU aterrice el endpoint, basta cambiar `setMarkLiveNotice(...)` por `await markTenantLive(...)` + un `useConfirm()` previo.
...
- **Fecha:** 2026-05-15
- **Objetivo:** cuarta y última vista del rol Viewer en `admin-panel/src/features/viewer/conversations/`, aplicando el mockup `36 _ Lectura _ Conversaciones.html`. Es la **lista read-only de conversaciones del tenant**, gateada con `<RequirePermission capability="conversations.view" mode="R">` (la capability ya existía — viewer/agent/manager/admin/owner = R). El principio rector vuelve a ser **reusar el dominio**: la vista reusa `InboxList` (UI-009.1) verbatim y la capa de datos `useInboxData` verbatim — sólo se añadió a `InboxList` un prop opcional `showStartForm = true` (default preserva el comportamiento legacy de `OperationsDesk`) y un wrapper fino de hook que expone únicamente la rebanada read-only. **Esta entrega cierra el bloque UI-010 (Viewer, 4 subtareas) Y el roadmap UI-006..UI-010 de rediseño por rol** (Platform Owner, Owner/Admin, Manager, Agente, Viewer). La próxima tarea `PENDING` real es **UI-011 — Cross-cutting: ToastProvider, ConfirmDialog, ErrorBoundary, reemplazo de `window.alert`/`window.confirm`**.
- **Cambios realizados:**
  - **Frontend — `src/features/viewer/conversations/` (nuevo, 7 archivos):**
    - `viewerConversationsData.js` (helpers puros, ~90 LOC): `CONVERSATIONS_DESCRIPTION` (subtítulo del PageHeader, menciona «solo lectura» y que los Viewers no pueden iniciar conversaciones ni tomar handoff), `countByStatus` (agrega por estado canónico, omite filas sin `status`), `countWithHandoff` (cuenta `human_required` + `human_active` + `waiting_agent`), `countUrgent` (cuenta solo `metadata.qualification.urgency_level` ∈ {emergency, high}; reusa `isUrgentConversation` de `inboxData.js`), `summarizeConversations(conversations, complaints)` (devuelve `{ total, withHandoff, urgent, complaints }` — usado en el chip de resumen del `PageHeader`). Re-exporta `parseMeta` para consumo conveniente.
    - `hooks/useViewerConversationsData.js` (~48 LOC): wrapper fino sobre `useInboxData({ session, tenant })`. **NO re-consulta nada** — reusa verbatim la lista de conversaciones, la lista de quejas, el filtro de inbox y el stream WebSocket de tiempo real del Inbox de Agente. **Expone solo la rebanada read-only**: `conversations`, `complaints`, `inboxFilter`, `summary` y el setter `setInboxFilter`. NO expone `startForm` / `acceptHandoff` / `releaseHandoff` / message composer state — los Viewers no obtienen ningún CTA de escritura (criterio global UI-010).
    - `ViewerConversations.jsx` (orquestador, ~83 LOC): monta `<RequirePermission capability="conversations.view" mode="R">` + `PageHeader` («Conversaciones» + chip de resumen «N conversaciones · M con handoff») + `<InboxList showStartForm={false} ... />`. `selectedConversationId={null}` y `onSelectConversation` es un no-op (los Viewers no abren el detalle; el orquestador no monta `ConversationView` / `MessageComposer` / `ContactSidePanel`). Toma `{ module, session, tenant }` por props; deriva `permissions` con `usePermissions({ profile, tenant })` + `useTenantContext()`.
    - `ViewerConversations.module.css` (~22 LOC): tokens 100% `var(--...)` — sin literales hex, sin `--success`/`--r-pill` (uso `--ok`/`--r-xl` cuando aplica). Mirror del `summaryChip` de `ViewerSummary.module.css`.
    - `index.js` (barrel: `export { ViewerConversations }`).
    - `viewerConversationsData.test.js` (5 tests, ~99 LOC): `CONVERSATIONS_DESCRIPTION` menciona «solo lectura»; `countByStatus` agrega y omite filas sin `status`; `countWithHandoff` cuenta los tres estados de handoff (`human_required`, `human_active`, `waiting_agent`); `countUrgent` cuenta solo `emergency`/`high` y soporta `metadata` stringificado (reusa `isUrgentConversation`); `summarizeConversations` agrega total/withHandoff/urgent/complaints y degrada a 0 con inputs inválidos.
    - `ViewerConversations.test.jsx` (4 tests, ~137 LOC — el archivo más grande del feature): render de la lista + tabs «Todas (N)» / «Quejas (M)» para un viewer y `listConversations` llamada con `(SESSION, 'tenant-acme')`; cambio a la tab de quejas muestra «No hay quejas activas en este tenant.»; **assert «zero write buttons»** — `queryByRole('button', { name: /iniciar conversación|enviar|tomar|liberar|aceptar handoff/i })` devuelve `null`, cubriendo start-form (no se renderiza por `showStartForm={false}`) / composer (no se monta) / handoff CTAs (no se montan) y verifica que las tabs `Todas`/`Quejas` SÍ existen (regex tight enough); `AccessDenied` cuando el rol del tenant carece de `conversations.view` y los botones de write tampoco se montan en ese caso. Mock de `coreApi.js` con todos los exports que importa `useInboxData` (incluido `openConversationStream` devolviendo un fake socket con `close()` para que el efecto del stream sea inerte) y de `TenantProvider.jsx`, siguiendo el patrón de `ViewerAppointments.test.jsx` / `OperationsDesk.test.jsx`.
  - **Frontend — `InboxList` ampliado con prop `showStartForm`:**
    - `src/features/agente/inbox/components/InboxList.jsx`: añadido prop opcional `showStartForm = true` (default preserva el comportamiento legacy). Cuando es `false`, el bloque `<form className="start-conversation-panel">` no se renderiza. Es una adición pura — el JSDoc se actualizó para documentar el nuevo prop y los parámetros que pasan a ser opcionales (`startForm` / `onStartFormChange` / `onStartConversation` solo se requieren cuando `showStartForm` es true). `OperationsDesk.jsx` no se tocó: sigue pasando los mismos props que antes y el default `true` mantiene el comportamiento idéntico. Se eligió la **opción 1** del brief (widening minimal del componente, ~5 LOC + actualización de docs) sobre el fallback de un `ViewerInboxList` paralelo: no hay duplicación de código y los tests del Inbox (`OperationsDesk.test.jsx` ×5) siguen verdes sin cambios.
  - **Frontend — wiring del nuevo módulo `viewer-conversations`:**
    - `app/moduleRegistry.js`: import de `ViewerConversations` + entrada `'viewer-conversations': { Component: ViewerConversations, capability: 'conversations.view' }`. La entrada legacy `operations-desk` se conserva: Owner/Admin/Manager/Agent siguen usándola via `TENANT_NAV` mapeada a `OperationsDesk` (Inbox de Agente — UI-009.1).
    - `data/modules.js`: entrada nueva `viewer-conversations` (label «Conversaciones», summary que describe la reutilización de `InboxList` + ausencia de composer/CTAs de handoff/start-form, scope, capability `conversations.view`).
    - `app/nav.js`: en `VIEWER_NAV` se reemplazó `'operations-desk'` por `'viewer-conversations'` en la sección «Lectura» (orden: `viewer-summary` → `viewer-analytics` → `viewer-appointments` → `viewer-conversations` → `contacts`). `TENANT_NAV` y `ROLE_HOME` **no** se tocaron — Owner/Admin/Manager/Agent siguen viendo `operations-desk` (la vista Inbox de Agente) y la landing del Viewer sigue siendo `viewer-summary` (UI-010.1).
    - `app/shells/ReadOnlyShell.test.jsx`: actualizado el botón que el Viewer ve en el shell — ahora es «Conversaciones» (no «Operations Desk»), porque `viewer-conversations` reemplazó a `operations-desk` en `VIEWER_NAV`.
- **Archivos modificados / creados:**
  - `admin-panel/src/features/viewer/conversations/{viewerConversationsData.js,viewerConversationsData.test.js,ViewerConversations.jsx,ViewerConversations.module.css,ViewerConversations.test.jsx,index.js}` (todos nuevos).
  - `admin-panel/src/features/viewer/conversations/hooks/useViewerConversationsData.js` (nuevo).
  - `admin-panel/src/features/agente/inbox/components/InboxList.jsx` (prop `showStartForm = true` + bloque condicional alrededor del `<form>` + JSDoc actualizado).
  - `admin-panel/src/app/moduleRegistry.js` (import + entrada `viewer-conversations`).
  - `admin-panel/src/app/nav.js` (`'operations-desk'` → `'viewer-conversations'` en `VIEWER_NAV` + comentario).
  - `admin-panel/src/data/modules.js` (entrada `viewer-conversations`).
  - `admin-panel/src/app/shells/ReadOnlyShell.test.jsx` (botón esperado: «Conversaciones»).
  - `docs/UI_BACKLOG.md` (UI-010.4 marcado `DONE` con bloques `Alcance` / `Diferencias intencionales declaradas` / `Tests`).
- **Validación local:**
  - `npm --prefix admin-panel run lint` → sin errores (solo los 2 warnings preexistentes de `tenant-setup`).
  - `npm --prefix admin-panel run build` → vite build OK (`✓ 415 modules transformed`).
  - `npm --prefix admin-panel test` → 95 archivos pasan, 1 falla (`src/app/router.test.jsx` con **exactamente 7 fallos** de entorno Node-24 `undici`/`AbortSignal`, documentados desde UI-002 / UI-006.* / UI-007.* / UI-008.* / UI-009.* / UI-010.1 / UI-010.2 / UI-010.3). **No es regresión** — el conteo coincide con la línea base; CI corre Node 20 y solo ejecuta lint + build para el front. Se añadieron **9 tests nuevos** (5 unit + 4 orchestrator) y todos pasan; 457 tests totales OK (vs 448 antes).
  - No se tocó ningún archivo de servidor → no se ejecutó pytest/ruff.
- **Seguridad:**
  - **Frontend-only** — no se tocó ningún archivo de servidor (`app/...`), schema, dependencia ni endpoint. La vista es una **lista read-only** que reusa la misma capa de datos del Inbox de Agente (`useInboxData` + `listConversations` + `listComplaintConversations`, todos endpoints de lectura ya **tenant-scoped server-side** vía header `X-Tenant-Id`).
  - La entrada de módulo se gateó con `<RequirePermission capability="conversations.view" mode="R">`; el backend reverifica con JWT + role + RLS. La capability ya existía en `permissions/matrix.js` (viewer/agent/manager/admin/owner = R) — no se añadieron nuevos permisos.
  - **No se renderiza ningún CTA de escritura**: (1) `showStartForm={false}` oculta el formulario «Iniciar conversación»; (2) el orquestador no monta `ConversationView` / `MessageComposer` / `ContactSidePanel` — el composer y las CTAs de handoff (`Tomar conversación`, `Liberar`, `Aceptar handoff`) no existen en el árbol renderizado; (3) las tabs `Todas` / `Quejas` son read-only por diseño (alternan qué subconjunto se pinta sin mutar nada en el servidor). El criterio global UI-010 («100% de las acciones write deben estar ocultas») se cumple por construcción; el test `ViewerConversations.test.jsx` documenta el contrato con `queryByRole('button', { name: /iniciar conversación|enviar|tomar|liberar|aceptar handoff/i })` debe ser `null`.
- **Limitaciones / próximos pasos:**
  - **Diferencias intencionales declaradas:** el HTML del mockup muestra (a) una columna derecha con el detalle del mensaje + thread del contacto, (b) chips de SLA por fila y (c) CTAs «Tomar handoff» / «Liberar» por fila. Esos elementos exigen montar el `ConversationView` (que arrastra el `MessageComposer` con write actions) y/o las acciones de handoff (`acceptHandoff` / `releaseHandoff`), **explícitamente prohibidas** por el criterio global UI-010 («100% de las acciones write deben estar ocultas»). Se difieren — no se inventan datos ni se cablea CTAs de escritura.
  - **Bloque UI-010 (Viewer) cerrado**: las 4 subtareas (`UI-010.1` Resumen, `UI-010.2` Analítica, `UI-010.3` Citas, `UI-010.4` Conversaciones) están DONE.
  - **Roadmap UI-006..UI-010 cerrado**: con esta entrega quedan completados todos los bloques de rediseño por rol — Platform Owner (UI-006), Owner/Admin (UI-007), Manager (UI-008), Agente (UI-009) y Viewer (UI-010).
  - **Próxima `PENDING` real:** **UI-011 — Cross-cutting: ToastProvider, ConfirmDialog, ErrorBoundary, reemplazo de `window.alert`/`window.confirm`**.

---

### UI-010.3 — Lectura · Citas (Viewer)

- **Fecha:** 2026-05-14
- **Objetivo:** tercera vista del rol Viewer en `admin-panel/src/features/viewer/appointments/`, aplicando el mockup `35 _ Lectura _ Citas.html`. Es el **listado paginado read-only de citas** del tenant, gateado con `<RequirePermission capability="appointments.view" mode="R">` (la capability ya existía — viewer/agent/manager/admin/owner = R). El principio rector vuelve a ser **reutilizar el dominio**: cada fila es un `AppointmentCard` (componente de dominio que ya mapea status → tono vía `appointmentStatusTone`), la paginación delega en el primitivo `Pagination`, los filtros usan `FormField` + `Card` + `CardHeader`. Con esta entrega arranca la tercera de las 4 subtareas del bloque UI-010; la próxima `PENDING` es **UI-010.4 — Lectura · Conversaciones (Viewer)**, la última del bloque.
- **Cambios realizados:**
  - **Frontend — `src/features/viewer/appointments/` (nuevo, 8 archivos):**
    - `viewerAppointmentsData.js` (helpers puros, ~165 LOC): `STATUS_FILTER_OPTIONS` (los cinco estados canónicos del backlog: confirmed / completed / pending / no_show / canceled), `statusLabel` (etiquetas en español con fallback), `dateKey` (clave local `YYYY-MM-DD`), `formatDateRange(appointment)` (es-CO `dateStyle:medium` + `timeStyle:short` sobre `starts_at`; degrada a `Sin fecha`), `filterAppointments` (cliente-side por status / rango de fechas inclusivo / query libre case-insensitive contra `contact_label`), `paginate(list, page, pageSize)` (1-based, clamp-ea la página al rango válido y devuelve `{ items, page, pageSize, total, totalPages }`), `csvCell` (escapa comas, comillas y saltos de línea), `toCsv(appointments)` (header `cliente,servicio,fecha,estado,recurso` + una fila por cita), `downloadCsv(csv, filename)` (Blob `text/csv;charset=utf-8` + `<a>` temporal; **único helper impuro** — la "export-to-CSV opcional" autorizada por el spec del backlog).
    - `hooks/useViewerAppointmentsData.js` (~115 LOC): owns el estado de filtros + página + lista cruda. Trae `listAppointments(session, tenantId)` sin parámetros server-side y filtra/pagina en cliente (idéntico patrón a `useTodayAppointmentsData` del feature `today-appointments`). `actions.exportCsv` corre sobre los registros **filtrados** (no sólo los paginados); `actions.setFilter(key, value)` resetea la página a 1.
    - `components/AppointmentsFilters.jsx` (~85 LOC): formulario denso con `FormField` (Estado select / Desde date / Hasta date / Cliente search) + acciones «Limpiar filtros» y «Exportar CSV». El submit del form hace `event.preventDefault()` (los filtros corren en cliente). El botón export queda deshabilitado si no hay tenant o `filteredCount === 0`.
    - `components/AppointmentsList.jsx` (~70 LOC): renderiza un `<ul>` de tarjetas `AppointmentCard` (cada una con `dateText={formatDateRange(appointment)}` y **sin `actions` slot ni `showPayment`** — criterio global UI-010). `Pagination` al final. EmptyState para `appointments.length === 0` y para loading inicial.
    - `ViewerAppointments.jsx` (orquestador, ~95 LOC): monta `<RequirePermission capability="appointments.view" mode="R">` + `PageHeader` («Citas» + descripción que menciona el banner read-only y la cadencia) + `AlertBanner` defensivo en caso de error + filtros + listado. Toma `{ module, session, tenant }` por props; deriva `permissions` con `usePermissions({ profile, tenant })` + `useTenantContext()`.
    - `ViewerAppointments.module.css` (~80 LOC): tokens 100% `var(--...)` — sin literales hex, sin `--success`/`--r-pill` (uso `--ok`/`--r-xl` cuando aplica). Mirror del vocabulario de `AuditPanel.module.css` (filterForm / filterGrid / filterActions / primaryButton / secondaryButton).
    - `index.js` (barrel: `export { ViewerAppointments }`).
    - `viewerAppointmentsData.test.js` (6 tests, ~130 LOC): `STATUS_FILTER_OPTIONS` + `statusLabel`, `filterAppointments` por status / rango / query, `paginate` con casos de frontera (lista vacía, clamp arriba/abajo), `csvCell` (escapa comas/comillas/saltos), `toCsv` (header + filas + estado mapeado por etiqueta), `formatDateRange` (degrada a `Sin fecha`).
    - `ViewerAppointments.test.jsx` (4 tests, ~155 LOC): render del listado + filtros + botón export para un viewer con `listAppointments` llamada sin filtros server-side; filtro de status reduce la lista visible (el `<select>` de Estado por label); click en «Exportar CSV» dispara `URL.createObjectURL` con un `Blob` `text/csv;charset=utf-8` (spy en jsdom + spy en `HTMLAnchorElement.prototype.click` para evitar abrir un download real) y `revokeObjectURL` se llama con la URL retornada; `AccessDenied` cuando el rol del tenant carece de `appointments.view` (verifica que el listado NO se monta y que `listAppointments` no se llamó). Mock de `coreApi.js` (sólo `listAppointments`) y de `TenantProvider.jsx`, siguiendo el patrón de `ViewerSummary.test.jsx`.
  - **Frontend — wiring del nuevo módulo `viewer-appointments`:**
    - `app/moduleRegistry.js`: import de `ViewerAppointments` + entrada `'viewer-appointments': { Component: ViewerAppointments, capability: 'appointments.view' }`. La entrada legacy `appointments` se conserva: Owner/Admin/Manager/Agent siguen viéndola via `TENANT_NAV` mapeada a `TodayAppointments` (vista Agente «Hoy · Citas del día», UI-009.5).
    - `data/modules.js`: entrada nueva `viewer-appointments` (label «Citas», summary que describe filtros + export CSV + sin acciones de fila, scope, capability `appointments.view`).
    - `app/nav.js`: en `VIEWER_NAV` se reemplazó `'appointments'` por `'viewer-appointments'` en la sección «Lectura» (orden: `viewer-summary` → `viewer-analytics` → `viewer-appointments` → `operations-desk` → `contacts`). `TENANT_NAV` y `ROLE_HOME` **no** se tocaron — Owner/Admin/Manager/Agent siguen viendo `appointments` (la vista Agente) y la landing del Viewer sigue siendo `viewer-summary`.
- **Archivos modificados / creados:**
  - `admin-panel/src/features/viewer/appointments/{viewerAppointmentsData.js,viewerAppointmentsData.test.js,ViewerAppointments.jsx,ViewerAppointments.module.css,ViewerAppointments.test.jsx,index.js}` (todos nuevos).
  - `admin-panel/src/features/viewer/appointments/hooks/useViewerAppointmentsData.js` (nuevo).
  - `admin-panel/src/features/viewer/appointments/components/{AppointmentsFilters.jsx,AppointmentsList.jsx}` (nuevos).
  - `admin-panel/src/app/moduleRegistry.js` (import + entrada `viewer-appointments`).
  - `admin-panel/src/app/nav.js` (`'appointments'` → `'viewer-appointments'` en `VIEWER_NAV` + comentario).
  - `admin-panel/src/data/modules.js` (entrada `viewer-appointments`).
  - `docs/UI_BACKLOG.md` (UI-010.3 marcado `DONE` con bloques `Alcance` / `Diferencias intencionales declaradas` / `Tests`).
- **Validación local:**
  - `npm --prefix admin-panel run lint` → sin errores nuevos (solo los warnings preexistentes de `tenant-setup`).
  - `npm --prefix admin-panel run build` → vite build OK.
  - `npm --prefix admin-panel test` → 91 archivos pasan, 1 falla (`src/app/router.test.jsx` con **exactamente 7 fallos** de entorno Node-24 `undici`/`AbortSignal`, documentados desde UI-002 / UI-006.* / UI-007.* / UI-008.* / UI-009.* / UI-010.1 / UI-010.2). **No es regresión** — el conteo coincide con la línea base; CI corre Node 20 y solo ejecuta lint + build para el front. Se añadieron **10 tests nuevos** (6 unit + 4 orchestrator) y todos pasan.
  - No se tocó ningún archivo de servidor → no se ejecutó pytest/ruff.
- **Seguridad:**
  - **Frontend-only** — no se tocó ningún archivo de servidor (`app/...`), schema, dependencia ni endpoint. La vista es un **listado read-only paginado** que consume únicamente `listAppointments` (endpoint de lectura ya **tenant-scoped server-side** vía header `X-Tenant-Id`).
  - El filtro por status/rango/query corre **en cliente** sobre la misma respuesta tenant-scoped — `listAppointments` no expone parámetros de fecha establecidos en el frontend; mismo patrón que la vista Agente «Hoy · Citas del día» (UI-009.5).
  - La entrada de módulo se gateó con `<RequirePermission capability="appointments.view" mode="R">`; el backend reverifica con JWT + role + RLS. La capability ya existía en `permissions/matrix.js` (viewer/agent/manager/admin/owner = R) — no se añadieron nuevos permisos.
  - **No se renderiza ningún CTA de escritura**: el `AppointmentCard` se monta sin el slot `actions` y con `showPayment={false}`; el único botón del feature distinto de los selectores de filtro es «Exportar CSV», que la spec del backlog autoriza explícitamente («solo filtros y export-to-CSV opcional») — el helper `downloadCsv` corre 100% en cliente sobre la respuesta ya autorizada por el backend, no llama a ningún endpoint write. El criterio global UI-010 («100% de las acciones write deben estar ocultas») se cumple por construcción; los tests documentan ese contrato (aserciones `queryByRole('button', { name: /Tomar handoff|Reagendar|Confirmar/i })` deben ser `null`).
- **Limitaciones / próximos pasos:**
  - **Diferencias intencionales declaradas:** el HTML del mockup muestra (a) un gráfico de «Citas por día · última semana» con día pico, (b) KPIs de «Atendidas / No-shows / Canceladas / Ticket promedio», (c) un panel «Estado de las citas · mayo» en proporción, (d) un bloque «Próximas citas destacadas» con miniaturas y (e) un CTA «Tomar handoff» por fila. Esos elementos asumen agregaciones (proyección, ocupación, ticket promedio) y acciones de escritura que **no existen en el endpoint `listAppointments`** (devuelve registros planos con `status`, `starts_at`, `service_name`/`service_code`, `resource_name` y `contact_label`). Se difieren — no se inventan datos ni se cablea CTAs de escritura, en línea con el criterio global UI-010.
  - **Próxima `PENDING` real:** **UI-010.4 — Lectura · Conversaciones (Viewer)**, la última subtarea del bloque UI-010.
