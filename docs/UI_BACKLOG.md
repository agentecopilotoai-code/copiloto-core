...
- **Estado:** PENDING (backend + frontend) — workaround documentado vía `BOOTSTRAP_PLATFORM_OWNER_SUPPORT_MODE=true` en `scripts/configure-auth0.sh`.
- **Síntoma:** desde `/admin/platform/tenants` (vista Platform Owner), el botón "Ver como tenant" en `FleetDrawer` invoca `handleSupportInto(tenant)` (`admin-panel/src/features/platform/fleet-tenants/FleetTenants.jsx:39-47`) que solo **navega** a `/admin/t/{slug}` — NO activa `support_mode` en ningún lado. Si el platform_owner no tiene `app_metadata.support_mode=true` permanente en su Auth0 user, `resolveActiveRoles({profile, tenant})` devuelve `[]` (TASK-0077 requiere support_mode para que roles globales apliquen en contexto de tenant) → "Sin acceso a ningún módulo".
- **Workaround actual:** `scripts/configure-auth0.sh` con `BOOTSTRAP_PLATFORM_OWNER_EMAIL=...` setea `support_mode=true` PERMANENTE en `app_metadata`. Funciona pero contradice TASK-0077 (opt-in temporal). Documentado en `INSTALL.md` § 4 Paso 3.2 con caveat operacional.
- **Fix correcto:**
  - **Backend** — nuevo endpoint `POST /v1/me/support-mode/{tenant_id}` montado en `me_router`:
    - Requiere `authenticate_request` + validación que `request.state.actor_type == 'user'` + check global `platform_owner` en `request.state.roles`.
    - Emite un cookie de sesión scoped al `tenant_id` con TTL (default 1h, configurable via `SUPPORT_MODE_TTL_MINUTES`).
    - Audit log obligatorio `support_mode.activated` con `metadata={tenant_id, expires_at, justification}` — campo `justification` requerido en el body para forensia post-mortem.
  - **Backend `authenticate_request`** — lee la cookie de support_mode al validar el JWT; si el `tenant_id` del cookie matchea con `X-Tenant-Id` o `request.state.tenant_id`, bumpea `request.state.support_mode = true` SOLO para ese tenant. No persiste el cambio en el JWT — es side-channel del cookie, scoped, temporal.
  - **Backend `DELETE /v1/me/support-mode/{tenant_id}`** — cancela el cookie antes del TTL. Audit log `support_mode.deactivated`.
  - **Frontend `handleSupportInto`** — antes de navegar a `/admin/t/{slug}`, invocar `POST /v1/me/support-mode/{tenant_id}` y solo navegar si responde 201. Mostrar un banner persistente en el header del tenant cuando `supportModeOverride === true` para visibilidad operacional.
  - **Frontend `resolveActiveRoles`** — leer el nuevo flag desde el contexto del panel (`supportModeOverride` del provider) además de `profile.support_mode`. Si cualquiera de los dos es true Y hay rol global, aplicar.
- **Tests:** cross-tenant escalation gate (platform_owner SIN cookie → 403 en `/v1/tenants/{tenant_id_ajeno}/...`), audit emitido, TTL respetado, cookie scoped (no aplica a otros tenant_ids).
- **Severidad:** alta operacional — sin esto el platform_owner SIEMPRE tiene `support_mode=true` permanente (workaround actual), lo que duplica el blast radius si robaran su cookie de sesión.
- **Dependencias:** ninguna — standalone backend + frontend cambio.
